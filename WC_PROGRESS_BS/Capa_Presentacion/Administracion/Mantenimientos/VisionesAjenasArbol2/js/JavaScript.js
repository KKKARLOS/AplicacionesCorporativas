    ...
    $('#divVisualizados .list-arrows button').on('click', function () {
        ...
        if ($button.hasClass('move-left')) {
            ...
            if (actives.length > 0) {
                ...
                //ordenar2($('#divlisPersonas.list-left ul li'));
                bCambios = true;
            } else {
            ...
            if (actives.length > 0) {
                ...
                //ordenar2($('#divlisVisualizados.list-right ul li'));
                bCambios = true;
            } else {
                alertNew("warning", "Tienes que seleccionar algún profesional");
            }
    ...
    try {
        ...
        for (var i = 0; i < datos.length; i++) {
            ...
            if (ficepiPadre != datos[i].Idficepi_visualizador) {
                ...
                //liVisionesAjeasArbol += "<span data-sexo='" + datos[i].Sexo_Visualizador + "' id='" + datos[i].Idficepi_visualizador + "' class='fk-vision'><i style='margin-right:4px' class='glyphicon-plus'></i>" + datos[i].Nombre_visualizador + "</span>";
            ...
            liVisionesAjeasArbol += "<li><span data-idficepivisualizado='" + datos[i].Idficepi_visualizado + "' style='margin-bottom: 10px; width:400px' class='profesional'>" + datos[i].Nombre_visualizado + "</span></li>";
            antTipo = "H";
...
$("#delVisualizador").on("click", function () {
    ...
    if ($("span.active").length > 0) {
        ...
        alertNew("warning", "No tienes ningún visualizador/a agregado/a seleccionado.");
    }
})
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if ($('#evaluador').attr("data-sexo") == "V") {
                $("#lblVisualizadorModal").text("Visualizador agregado");
                $("#spanVisualizador").text("Visualizador agregado");
            }
            else {
                $("#lblVisualizadorModal").text("Visualizadora agregada");
                $("#spanVisualizador").text("Visualizadora agregada");
            }
...
$('#modal-seleccionVisualizados').on('shown.bs.modal', function () {
    ...
    $("#Headermodal-seleccionVisualizados").text("Selección de visualizadores/as de " + $("#evaluador").val());
...
$("#mntVisualizados").on("click", function () {
    if ($("#evaluador").val() == "") {
        alertNew("warning", "Para seleccionar visualizadores/as, tienes que tener seleccionado algún visualizador/a agregado/a.");
        return;
    }
    else {
        $tablaGuardada = $("#tabla");
...
$("#editVisualizador").on("click", function () {
    ...
    if ($(".fk-vision + span.active").length == 0) {
        alertNew("warning", "Para modificar, tienes que tener seleccionado algún visualizador/a agregado/a.");
        return;
    }
