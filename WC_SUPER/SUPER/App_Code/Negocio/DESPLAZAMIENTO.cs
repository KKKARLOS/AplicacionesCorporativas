...
namespace SUPER.BLL
{
    public partial class DESPLAZAMIENTO
    {
        ...
        public static string ObtenerDesplazamientosRangoFechas(int t314_idusuario, DateTime fec_desde, DateTime fec_hasta, int t420_idreferencia)
        {
            ...
            sb.Append("<table id='tblDatos' style='width:750px;cursor:url(../images/imgManoAzul2.cur),pointer;' class='texto'>");
            sb.Append("<colgroup>");
            ...
            SqlDataReader dr = SUPER.Capa_Datos.DESPLAZAMIENTO.ObtenerDesplazamientos(null, t314_idusuario, fec_desde, fec_hasta, t420_idreferencia);
            while (dr.Read())
            {
                sb.Append("<tr style='height:20px;' id=\"" + dr["t615_iddesplazamiento"].ToString() + "\" ");
                sb.Append("ondblclick='aceptarClick(this)'>");

                sb.Append("<td style='padding-right:10px; text-align:right;'>" + int.Parse(dr["t615_iddesplazamiento"].ToString()).ToString("#,###") + "</td>");
                ...
                sb.Append("<td style='padding-right:2px; text-align:right;'>" + dr["numero_usos"].ToString() + "</td>");
                sb.Append("</tr>");
            }
            dr.Close();
            dr.Dispose();
