...
namespace SUPER.BLL
{
    ...
    public partial class EXPPROFACT
    {
        ...
        public static string getAreas(SqlTransaction tr, int t808_idexpprof, bool bModoLectura)
        {
            ...
            sb.Append("<table id='tblConTec'");
            if (bModoLectura)
                sb.Append(" class='texto' style='width:420px; table-layout:fixed; border-collapse: collapse;' cellspacing='0' border='0'>");
            else
                sb.Append(" class='texto MANO' style='width:420px; table-layout:fixed; border-collapse: collapse;' cellspacing='0' border='0'>");
            sb.Append("<colgroup>");
            ...
            if (t808_idexpprof != -1)
            {
                SqlDataReader dr = SUPER.DAL.EXPPROFACT.getAreas(null, t808_idexpprof);
                while (dr.Read())
                {
                    sb.Append("<tr id='" + dr["t810_idacontecno"].ToString() + "' bd='' ");
                    if (bModoLectura)
                    ...
                    sb.Append("<td><nobr class='NBR W390'>" + dr["t810_denominacion"].ToString() + "</nobr></td>");
                    sb.Append("</tr>");
                }
                dr.Close();
                dr.Dispose();
        ...
        public static void InsertEP(SqlTransaction tr, int t808_idexpprof, string sAct)
        {
            ...
            SUPER.DAL.EXPPROFACT.InsertEP(tr, t808_idexpprof, sAct);
        }
