...
function dragDropEnd(e) {
    ...
    if (destinationObj.tagName == "UL" || destinationObj.tagName == "IMG" || destinationObj.tagName == "LI") {
        ...
        switch (destinationObj.tagName) {
            ...
            case "IMG":
                ...
                if (destinationObj.src.indexOf("imgAdministador.gif") == -1
	                && destinationObj.src.indexOf("imgEncargadoCV.gif") == -1
	                && destinationObj.src.indexOf("imgConsultorCV.gif") == -1) {
                    bEliminar = true;
                }
                break;
        }
    ...
    if (bEliminar) {
        ...
        if (strOrigenIcono != "selector") {
            if (oID == "C") {
                borrarConsultor(objFilaActiva);
            }
            try {
                if (objFilaActiva != null) {
                    fm_mn(objFilaActiva);
                }
                objFilaActiva = null;
            } catch (e) { }
    ...
    if (cloneSourceItems && (!destinationObj || (destinationObj && (destinationObj.id == 'allItems' || destinationObj.parentNode.id == 'allItems')))) {
        ...
        if (destinationObj) {
            ...
            //Inserta en la posición correcta para evitar tener que reordenar después
            if (oUL.children.length != 0){
                if (oUL.children[oUL.children.length - 1].value > contentToBeDragged.value) {
                    for (i = 0; i < oUL.children.length; i++) {
                        if (oUL.children[i].value > contentToBeDragged.value) {
                            oUL.insertBefore(contentToBeDragged, oUL.children[i]);
                            break;
                        }
                    } 
                }else
            ...
            var oFila = oUL.parentNode.parentNode.parentNode;
            if (contentToBeDragged.id == "C") {
                crearConsultor(oFila);
            }
            //Seleccionar la fila donde hemos añadido figura
            ms(oFila);
            cargarDatosConsultor(oFila);
...
function initDragDropScript() {
    ...
    for (var no = 0; no < listItems.length; no++) {
        if (typeof document.attachEvent != 'undefined') {
            ...
            listItems[no].addEventListener("mousedown", initDrag, false);
            listItems[no].addEventListener("selectstart", cancelEvent, false);
        }
    ...
    if (typeof document.attachEvent != 'undefined') {
        document.documentElement.attachEvent("onmousemove", moveDragContent);
        document.documentElement.attachEvent("onmouseup", dragDropEnd);
    } else {
        document.addEventListener("mousemove", moveDragContent, false);
        document.addEventListener("mouseup", dragDropEnd, false);
    }
