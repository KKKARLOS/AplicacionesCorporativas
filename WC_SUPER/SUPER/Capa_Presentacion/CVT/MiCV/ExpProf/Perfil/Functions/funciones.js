...
function datosGenericos() {
    ...
    //JLG (01-16): Si grabamos siendo plantila siempre se va a realizar una insert en perfil
    if ($I("hdnPlant").value != "")
        sb.Append("-1##");
    else
    ...
    //JLG (01-16): Se añade este campo para saber si el origen es una plantilla
    sb.Append($I("hdnPlant").value + "##");
    sb.Append($I("hdnLigadoProy").value);
    return sb.ToString();
}
function datosET() {
    ...
    //JLG (01-16): Si el origen es una plantilla, siempre se van a insertar en t815
    if ($I("hdnPlant").value != "")
    {
...
function comprobarDatos() {
    ...
    //2016/01/07 Mikel. Para perfiles en experiencias no ligadas a proyecto no tiene sentido controlar la fecha en la experiencia
    //pues las fechas las marcan los perfiles
    if ($I("hdnLigadoProy").value == "S") {
...
function salir() {
    ...
    //JLG (01-16): En el caso de plantilla también se puede grabar
...
function anadirCON(tipo){
    try {
        ...
        var s  = strServer + "Capa_Presentacion/CVT/MiCV/ExpProf/Perfil/getConocimientos.aspx?tipo=" + tipo + "&cono=" + cono;
