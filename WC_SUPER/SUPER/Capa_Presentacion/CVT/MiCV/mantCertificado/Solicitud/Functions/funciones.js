...
function comprobarDatos(){
    try{
        ...
        var aFila = FilasDe("tblDocumentos");
        if ($I("tblDocumentos") == null || aFila == null || aFila.length == 0) {
            mmoff("War", "Debes adjuntar algún documento", 250);
            return false;
        }
        return true;
	}catch(e){
...
function grabar(){
    try{
        ...
        var js_args = "grabar@#@" + $I("hdnTipo").value + "##" + Utilidades.escape($I("txtDen").value) + "##" + 
                                    Utilidades.escape($I("txtObs").value) + "##" + sIDDocuAux + "##" + $I("hdnIdFicepicert").value;
