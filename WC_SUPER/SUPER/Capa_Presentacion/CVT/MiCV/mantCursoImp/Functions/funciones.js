...
//    if (nName == 'chrome')
//        if ($I("hdnOP").value == "1")
//            window.returnValue = { resultado: "OK", bDatosModificados: true, id: $I("hdnIdCurMonitor").value }
}
...
function salirContinuar() {
    var returnValue = null;
    if (bHayCambios) returnValue = $I("hdnIdCurso").value;
    modalDialog.Close(window, returnValue);
}
    ...
    try {
        ...
        if ($I("hdnEstadoNuevo").value != "B" && $I("hdnEsEncargado").value == "0") {
            ...
            if ($I("query").value.Trim() == "" || $I("query").value == "Ej: MICROSOFT IBERICA,S.R.L.") {
                ocultarProcesando();
                mmoff("War", "El centro de impartición del curso es dato obligatorio.", 380, 1500);
                return false;
            }
        }
        ...
        if ($I("hdnEstadoNuevo").value != "B" && $I("hdnEsEncargado").value == "0") {
            if ($I("txtNombreDocumento").value.Trim() == "") {//Si el combo esta deshabilitado, se trata de un titulo ya existente que ya posee documento y que sólo podrá modificarlo por otro, por lo que no nos tenemos que preocupar en mirar si el campo está vacio o no
                ...
                if ($I("txtObservaciones").value.Trim() == "") {
                    mmoff("War", "El documento acreditativo del curso es dato obligatorio, a menos que se justifique su ausencia en el campo de \"Observaciones\".", 350, 5000);
                } else {
                    ocultarProcesando();
...
function LlamarGrabar() {
    try {
        var js_args = "grabar@#@" + $I("hdnIdCurMonitor").value + "@#@";
...
function verDOC() {
    ...
    try {
        ...
        if (!re.test($I("hdnndoc").value)) {
            if ($I("hdnIdCurMonitor").value == "-1")
                descargar('CVTCUR_IMP', $I("hdnUsuTick").value + "datos" + $I("hdnIdFicepi").value);
            else
                descargar('CVTCUR_IMP', $I("hdnIdCurso").value + "datos" + $I("hdnIdFicepi").value);
        }
        else
            descargarhtm('CVTCUR_IMP', $I("hdnIdCurso").value + "datos" + $I("hdnIdFicepi").value);
    } catch (e) {
...
//    try {
//        if ($I("hdnIdCurMonitor").value == "-1") {
//            nuevoDoc("CURSOI", sIDDocuAux, $I("hdnIdFicepi").value);
//        } else {
//            if ($I("hdnContentServer").value == "")
//                nuevoDoc("CURSOI", $I("hdnIdCurMonitor").value, $I("hdnIdFicepi").value);
//            else
//                modificarDoc("CURSOI", $I("hdnIdCurMonitor").value, $I("hdnIdFicepi").value);
//        }
...
function getHistorial() {
    try {
        mostrarProcesando();
        var strEnlace = strServer + "Capa_Presentacion/getCronologia/Default.aspx?t=" + codpar('CURMONITORCRONO') + "&k=" + codpar($I("hdnIdCurMonitor").value);
