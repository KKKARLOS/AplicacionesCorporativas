...
function RedimensionarColumnas(sIDTabla) {
    //var tables = document.getElementsByTagName('table');
    //for (var i = 0; tables.item(i); i++) {
        if ($I(sIDTabla).className.match(/resizable/)) {
            // generate id
            if (!$I(sIDTabla).id) $I(sIDTabla).id = 'table' + (i + 1);
            // make table resizable
            resizableTables[resizableTables.length] = new ColumnResize($I(sIDTabla));
        }
    //}
    //	alert(resizableTables.length + ' tables was added.');
}
