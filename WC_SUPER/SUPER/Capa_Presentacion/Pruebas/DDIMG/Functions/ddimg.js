...
function saveDragDropNodes()
{
	...
	document.getElementById('saveContent').innerHTML = '<h3>Ready to save these nodes:</h3> ' + saveString.replace(/;/g,';<br>') + '<p>Format: ID of ul |(pipe) ID of li;(semicolon)</p><p>You can put these values into a hidden form fields, post it to the server and explode the submitted value there</p>';
}
...
function initDragDropScript()
{
	...
	for(var no=0;no<ulArray.length;no++){
		...
		ulPositionArray[no]['left'] = ulArray[no].offsetParent.offsetLeft; //getLeftPos(ulArray[no]);	
		ulPositionArray[no]['top'] = ulArray[no].offsetParent.offsetTop; //getTopPos(ulArray[no]);	
