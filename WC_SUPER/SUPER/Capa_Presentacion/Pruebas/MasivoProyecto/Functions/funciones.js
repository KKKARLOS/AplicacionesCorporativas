...
function obtenerDatosExcel()
{
    ...
    {   
        ...
        {
            mmoff("Inf", "Rango de fechas incorrecto o debe indicar las fechas.", 370);
	        return false;
        };
...
function getDatosTabla(nTipo){
    try{
        ...
        switch (nTipo)
        {
            case 1: oTabla = tblAmbito; break;
            case 2: oTabla = tblResponsable; break;
            case 3: oTabla = tblNaturaleza; break;
            case 4: oTabla = tblModeloCon; break;
            case 5: oTabla = tblHorizontal; break;
            case 6: oTabla = tblSector; break;
            case 7: oTabla = tblSegmento; break;
            case 8: oTabla = tblCliente; break;
            case 9: oTabla = tblContrato; break;
            case 10: oTabla = tblQn; break;
            case 11: oTabla = tblQ1; break;
            case 12: oTabla = tblQ2; break;
            case 13: oTabla = tblQ3; break;
            case 14: oTabla = tblQ4; break;
            case 16: oTabla = tblProyecto; break;
        }
...
//	var oF1 = new Date();  
	...
	{	
	    ...
	    //oTableOrig = oTarget.getElementsByTagName("TABLE")[0].cloneNode(true);
	    if (!!oTable) oTableOrig = oTable.cloneNode(true);
	    for (var x=0; x<=aEl.length-1;x++){
	        ...
	        switch(oTarget.id){
		        ...
		        case "ctl00_CPHC_divCatalogo2":
                    ...
                    for (var i=0;i<oTableOrig.rows.length;i++){
	                    if (oTableOrig.rows[i].id == oRow.id){
		                    sw = 1;
		                    break;
	                    }
                    }
...
//                        nIndiceInsert++;
                        ...
                        NewRow.swapNode(oCloneNode);
						oCloneNode.className = "MM";
						oCloneNode.cells[3].className = "MM";
    ...
    try{
        ...
        switch (nTipo){
            ...
            case 16:
                ...
                if (es_administrador == "") strEnlace = strServer + "Capa_Presentacion/ECO/Consultas/getCriterioProyecto/Default.aspx?nTipo=" + nTipo;
                else{
                    strEnlace = strServer + "Capa_Presentacion/PSP/Informes/Conceptos/Proyecto/Default.aspx?sMod=pst";
                    sTamano = "dialogwidth:1010px; dialogheight:720px; center:yes; status:NO; help:NO;";
                }
                break;
            default:
                if (es_administrador == "") strEnlace = "../../ECO/Consultas/getCriterio/Default.aspx?nTipo="+nTipo;
                else strEnlace = strServer + "Capa_Presentacion/ECO/Consultas/getCriterioTabla/default.aspx?nTipo=" + nTipo;
                break;
        }   
	                ...
	                switch (nTipo) {
	                        ...
	                        for (var i = 1; i < aElementos.length; i++) {
	                            ...
	                            switch (parseInt(oNF.tipo, 10)) {
	                                case 1:
	                                    ...
	                                    break;
	                                case 2:
	                                    oNF.insertCell(-1).appendChild(document.createElement("<img src='../../../Images/imgSN3.gif' style='margin-left:2px;margin-right:4px;vertical-align:middle;border: 0px;'>"));
	                                    oNF.id = aID[1];
	                                    break;
	                                case 3:
	                            ...
	                            oNF.cells[0].children[1].innerHTML = aDatos[2];
	                        }
	                    ...
	                    case 16:
	                        ...
	                        for (var i = 0; i < aElementos.length; i++) {
	                            if (aElementos[i] == "") continue;
	                            var aDatos = aElementos[i].split("@#@");
	                            var oNF = tblProyecto.insertRow(-1);
	                            oNF.id = aDatos[0];
	                            oNF.style.height = "16px";
	                            oNF.categoria = aDatos[2];
	                            oNF.cualidad = aDatos[3];
	                            oNF.estado = aDatos[4];
	                            oNF.insertCell(-1);
        ...
        for (var n=1; n<=16; n++){
            ...
            for (var i=0; i<oTabla.rows.length;i++){
                ...
                if (n==1){
                    ...
                    sb.Append(n +"##"+ oTabla.rows[i].tipo+"-"+oTabla.rows[i].id +"##"+ Utilidades.escape(oTabla.rows[i].innerText));
                }else if (n==16){
                    if (sb.buffer.length>0) sb.Append("///");
                    sb.Append(n +"##"+ oTabla.rows[i].id+"-"+oTabla.rows[i].categoria+"-"+oTabla.rows[i].cualidad+"-"+oTabla.rows[i].estado +"##"+ Utilidades.escape(oTabla.rows[i].innerText));
                }else{
                    if (sb.buffer.length>0) sb.Append("///");
