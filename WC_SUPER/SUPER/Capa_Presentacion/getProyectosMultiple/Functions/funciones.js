    ...
    try{
        ...
        for (var i=0; i< $I("tblDatos").rows.length; i++){
            ...
            if ($I("tblDatos").rows[i].className == "FS"){
                sPSN += $I("tblDatos").rows[i].getAttribute("idPSN") +",";
                sPE += $I("tblDatos").rows[i].getAttribute("idPE") +",";
            }
        }
        var returnValue = null;
        if (sw != 0){
            sPSN = sPSN.substring(0, sPSN.length - 1);
            sPE = sPE.substring(0, sPE.length - 1);
            returnValue = sPSN +"@#@"+ sPE;
        }
...
function aceptarSalir(){
    try{
        ...
        mostrarErrorAplicacion("Error al aceptar y salir", e.message);
    }
}
...
//                break;
            ...
            case "setPreferencia":
                if (aResul[2] != "0")
                    mmoff("Suc", "Preferencia almacenada con referencia: " + aResul[2].ToString("N", 9, 0), 300, 3000);
                else
                    mmoff("War", "La preferencia a almacenar ya se encuentra registrada.", 350, 3000);
                break;
            case "delPreferencia":
...
function getCualificador(sOpcion){
    try{
        ...
        switch(sOpcion){
            ...
            case "Q1": strEnlace += "getCNP.aspx?sTipo="+ sOpcion +"&idNodo="+ sValorNodoe +"&sTitulo="+ $I("lblCSN1P").innerText; break;
