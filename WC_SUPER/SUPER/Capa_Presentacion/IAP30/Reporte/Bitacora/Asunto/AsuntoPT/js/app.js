$(document).ready(function () { SUPER.IAP30.BitacoraAsuntoPT.app.init(); })

var SUPER = SUPER || {};
SUPER.IAP30 = SUPER.IAP30 || {};
SUPER.IAP30.BitacoraAsuntoPT = SUPER.IAP30.BitacoraAsuntoPT || {}

SUPER.IAP30.BitacoraAsuntoPT.app = (function (view) {
    ...
    buscarProfesionales = function (event) {
        ...
        if (view.dom.Apellido1.val() == "" && view.dom.Apellido2.val() == "" && view.dom.Nombre.val() == "") {
            IB.bsalert.fixedAlert("warning", "Error de aplicación", "No se permite realizar búsquedas con los filtros vacíos");
            return false;
        }
    ...
    grabarDatos = function (e) {
        if (comprobarDatosGrabar()) {
            ...
            var DatosGenerales = {
                ...
                t331_idpt: parseInt(IB.vars.nPT),
                t305_idproyectosubnodo: parseInt(IB.vars.nPSN),
                T409_alerta: view.dom.alerta.val(),
                T409_desasunto: view.dom.desAsunto.val(),
                T409_desasuntolong: view.dom.descripcion.val(),
                T409_dpto: view.dom.departamento.val(),
                T409_estado: view.dom.estado.val(),
                T409_etp: parseFloat(getFloat(view.dom.esfuerzoPlanificado.val())),
                T409_etr: parseFloat(getFloat(view.dom.esfuerzoReal.val())),
                T409_fcreacion: cadenaAfecha(view.dom.fechaCreacion.val()),
                T409_ffin: (view.dom.fechaFinalizacion.val()=="")? null : cadenaAfecha(view.dom.fechaFinalizacion.val()),
                T409_flimite: (view.dom.fechaLimite.val() == "") ? null : cadenaAfecha(view.dom.fechaLimite.val()),
                T409_fnotificacion: cadenaAfecha(view.dom.fechaNotificacion.val()),
                T409_idasunto: parseInt(IB.vars.idAsunto),
                T409_notificador: view.dom.notificador.val(),
                T409_obs: view.dom.observaciones.val(),
                T409_prioridad: view.dom.prioridad.val(),
                T409_refexterna: view.dom.refExterma.val(),
                T409_registrador: parseInt(IB.vars.codUsu),
                T409_responsable: parseInt(IB.vars.idResponsable),
                T409_severidad: view.dom.severidad.val(),
                T409_sistema: view.dom.sistema.val(),
                ...
                T409_estado_anterior: IB.vars.coEstadoAnterior,
                DesPE: IB.vars.desPE,
            ...
            $.each($(view.selectores.filasProfesionalesSel), function () {
                //if ($(this).attr("data-bd") != "") {//Solo se actualizan filas modificadas // no porque sino luego no aparecen en la relación de notificación de correo
                    ...
                    oIntegrante.T409_idasunto = parseInt(IB.vars.idAsunto);
                    oIntegrante.t413_notificar = ($(this).children().eq(1).children().eq(0).is(':checked') == true) ? true : false;
   ...
   var controlCargaSolapas = function (e) {
    ...
    obtenerDatosCronologia = function (e) {
        if (IB.vars.idAsunto != "") {
            ...
            IB.DAL.post(null, "obtenerDetalleCronologia", filtro, null,
                ...
                function (ex, status) {
                    IB.bsalert.fixedAlert("danger", "Error de aplicación", "Error al obtener los datos de cronología del asunto (" + IB.vars.idAsunto + ").");
                    IB.procesando.ocultar();
                }
            );
        };
    ...
    function initPluginDocumentacion() {
        ...
        $.when(SUPER.IAP30.appDocumentos.initModuloDoc(_idDoc, "detalleAsuntoPT", _modoContainerDocs, IB.vars.codUsu, _soySuperEditor))
           .then(function () {
...
})(SUPER.IAP30.BitacoraAsuntoPT.View);