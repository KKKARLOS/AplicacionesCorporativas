$(document).ready(function () { SUPER.IAP30.BitacoraAsuntoPE.app.init(); })

var SUPER = SUPER || {};
SUPER.IAP30 = SUPER.IAP30 || {};
SUPER.IAP30.BitacoraAsuntoPE = SUPER.IAP30.BitacoraAsuntoPE || {}

SUPER.IAP30.BitacoraAsuntoPE.app = (function (view) {
    ...
    grabarDatos = function (e) {
        if (comprobarDatosGrabar()) {
            ...
            var DatosGenerales = {
                ...
                T382_alerta: view.dom.alerta.val(),
                T382_desasunto: view.dom.desAsunto.val(),
                T382_desasuntolong: view.dom.descripcion.val(),
                T382_dpto: view.dom.departamento.val(),
                T382_estado: view.dom.estado.val(),
                T382_etp: parseFloat(getFloat(view.dom.esfuerzoPlanificado.val())),
                T382_etr: parseFloat(getFloat(view.dom.esfuerzoReal.val())),
                T382_fcreacion: cadenaAfecha(view.dom.fechaCreacion.val()),
                T382_ffin: cadenaAfecha(view.dom.fechaFinalizacion.val()),
                T382_flimite: cadenaAfecha(view.dom.fechaLimite.val()),
                T382_fnotificacion: cadenaAfecha(view.dom.fechaNotificacion.val()),
                T382_idasunto: parseInt(IB.vars.idAsunto),
                T382_notificador: view.dom.notificador.val(),
                T382_obs: view.dom.observaciones.val(),
                T382_prioridad: view.dom.prioridad.val(),
                T382_refexterna: view.dom.refExterma.val(),
                T382_registrador: parseInt(IB.vars.codUsu),
                T382_responsable: parseInt(IB.vars.idResponsable),
                T382_severidad: view.dom.severidad.val(),
                T382_sistema: view.dom.sistema.val(),
                ...
                T382_estado_anterior: IB.vars.coEstadoAnterior,
                DesPE: IB.vars.desPE,
            ...
            $.each($(view.selectores.filasProfesionalesSel), function () {
                //if ($(this).attr("data-bd") != "") {//Solo se actualizan filas modificadas // no porque sino luego no aparecen en la relación de notificación de correo
                    ...
                    oIntegrante.T382_idasunto = parseInt(IB.vars.idAsunto);
                    oIntegrante.T388_notificar = ($(this).children().eq(1).children().eq(0).is(':checked') == true) ? true : false;
    ...
    var salir = function (e) {        
        $.when(controlarSalir(e)).then(function () {
            if (IB.vars.origen == "IAP")
                setTimeout(function () { location.href = "../../../../Consultas/Bitacora/PE/Default.aspx?" + IB.vars.qs; }, 500);
            else
   ...
   var controlCargaSolapas = function (e) {
    ...
    function initPluginDocumentacion() {
        ...
        $.when(SUPER.IAP30.appDocumentos.initModuloDoc(_idDoc, "detalleAsuntoPE", _modoContainerDocs, IB.vars.codUsu, _soySuperEditor))
           .then(function () {
...
})(SUPER.IAP30.BitacoraAsuntoPE.View);