...
SUPER.IAP30.BitacoraAccionPT.View = (function () {
    ...
    var init = function () {
        if (IB.vars.permiso == "L") {
            ...
            dom.btnNuevaTarea.hide();
            dom.btnDeshacerTarea.hide();
            dom.btnEliminarTarea.hide();
            dom.btnGrabar.hide();
        }
        else {
    ...
    pintarDatosAsunto = function (oAsunto) {
        ...
        if (IB.vars.idResponsable != "") IB.vars.idResponsable = oAsunto.T409_responsable;
        IB.vars.bCambios = 0;
    }

    pintarDatosAccion = function (oAccion) {
        dom.idAccion.val(accounting.formatNumber(oAccion.T410_idaccion,0));
        dom.desAccion.val(oAccion.T410_desaccion);
        dom.observaciones.val(oAccion.T410_obs);
        dom.descripcion.val(oAccion.T410_desaccionlong);
        dom.departamento.val(oAccion.T410_dpto);
        dom.alerta.val(oAccion.T410_alerta);
        if (oAccion.T410_flimite) dom.fechaLimite.val(moment(oAccion.T410_flimite).format("DD/MM/YYYY"));
        if (oAccion.T410_ffin) dom.fechaFinalizacion.val(moment(oAccion.T410_ffin).format("DD/MM/YYYY"));
        dom.avance.val(oAccion.T410_avance);
        dom.desAccion.focus();
    }
    ...
    function pintarProfesionalesAccion(data) {
        ...
        $.each(data, function (index, item) {
            ...
            sNotificacion = (item.t414_notificar) ? "checked=true" : "";
    ...
    function pintarTablaTareas(data) {
        ...
        $.each(data, function (index, item) {
            ...
            sContent = "<b>Proy. Eco.:</b> " + item.t301_denominacion.replace(/"/g, "") + "<br /> <b>Proy. Téc.:</b> " + item.t331_despt.replace(/"/g, "") + "<br />";
            if (item.t334_desfase != "") sContent += " <b>Fase:</b> " + item.t334_desfase.replace(/"/g, "") + "<br />";
