...
SUPER.IAP30.BitacoraAccionTarea.View = (function () {
    ...
    pintarDatosAsunto = function (oAsunto) {
        ...
        if (IB.vars.idResponsable != "") IB.vars.idResponsable = oAsunto.T600_responsable;
        IB.vars.bCambios = 0;
    }

    pintarDatosAccion = function (oAccion) {
        dom.idAccion.val(accounting.formatNumber(oAccion.T601_idaccion,0));
        dom.desAccion.val(oAccion.T601_desaccion);
        dom.observaciones.val(oAccion.T601_obs);
        dom.descripcion.val(oAccion.T601_desaccionlong);
        dom.departamento.val(oAccion.T601_dpto);
        dom.alerta.val(oAccion.T601_alerta);
        if (oAccion.T601_flimite) dom.fechaLimite.val(moment(oAccion.T601_flimite).format("DD/MM/YYYY"));
        if (oAccion.T601_ffin) dom.fechaFinalizacion.val(moment(oAccion.T601_ffin).format("DD/MM/YYYY"));
        dom.avance.val(oAccion.T601_avance);
        dom.desAccion.focus();
    }
    ...
    function pintarProfesionalesAccion(data) {
        ...
        $.each(data, function (index, item) {
            ...
            sNotificacion = (item.t605_notificar) ? "checked=true" : "";
    ...
    // Deshacer cambios realizados en tareas
    var deshacerCambiosTarea = function (e) {
        $.each($(selectores.filasSeleccionadasTareas), function (e) {
            if ($(this).attr("data-bd") == "D") {
                $(this).attr("data-bd", "");
                $(this).children().eq(1).removeClass("marcadoParaBorrar").addClass("desmarcarParaBorrar");
                $(this).children().eq(2).removeClass("marcadoParaBorrar").addClass("desmarcarParaBorrar");
                $(this).children().eq(3).removeClass("marcadoParaBorrar").addClass("desmarcarParaBorrar");
                $(this).children().eq(4).removeClass("marcadoParaBorrar").addClass("desmarcarParaBorrar");
                $(this).children().eq(5).removeClass("marcadoParaBorrar").addClass("desmarcarParaBorrar");
                $(this).children().eq(6).removeClass("marcadoParaBorrar").addClass("desmarcarParaBorrar");
                $(this).children().eq(7).removeClass("marcadoParaBorrar").addClass("desmarcarParaBorrar");
                $(this).children().eq(8).removeClass("marcadoParaBorrar").addClass("desmarcarParaBorrar");
