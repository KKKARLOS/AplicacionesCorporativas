	...
	dayMousedown: function(ev) {
		...
		var dragListener = new HitDragListener(this, {
			...
			hitOver: function(hit, isOrig, origHit) {
				//miren:Si es día pasado no se abre el detalle
				//TODO: En caso de pinchar en un evento pasado debería de abrirse el detalle
				if(!$(_this.getHitEl(origHit)).hasClass('fc-past')){
...
var DayTableMixin = FC.DayTableMixin = {
	...
	renderHeadDateCellHtml: function(date, colspan, otherAttrs) {
	    ...
	    var diaLaboral = ((date.day() - 1) == -1 ? diaLaboral = 6 : diaLaboral = date.day() - 1);
		return '' +
			'<th scope="row" class="fc-day-header ' + view.widgetHeaderClass + ' fc-' + dayIDs[date.day()] +
                                    (aSemanaLab[diaLaboral] == "0" ? ' fc-Festivo"' : '"') +
	...
	renderBgCellHtml: function(date, otherAttrs) {
		...
		return '<td tabindex="0" class="' + classes.join(' ') + '"' +
...
DayGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		if (seg.isStart) {
			timeText = this.getEventTimeText(event, 'LT');
			if (timeText) {
		...
		for (i = 0; i < levelCnt; i++) { // iterate through all levels
			...
			if (levelSegs) {
				for (j = 0; j < levelSegs.length; j++) { // iterate through segments in level
					...
					td = $('<td tabindex="0" class="fc-event-container"/>').append(seg.el);
...
var View = FC.View = Class.extend({
	...
	initHiddenDays: function() {
		...
		if (this.opt('weekends') === false) {
			hiddenDays.push(5, 6); // 0=sunday, 6=saturday
		}
...
function Header(calendar, options) {
	...
	function render() {
		...
		if (sections) {
		    el = $("<div class='fc-toolbar bg-primary'/>")
		...
		if (buttonStr) {
			$.each(buttonStr.split(' '), function(i) {
				...
				$.each(this.split(','), function(j, buttonName) {
					...
					if (buttonName == 'title') {
						groupChildren = groupChildren.add($('<h2 class="mayor">&nbsp;</h2>')); // we always want it to take up height
						isOnlyButtons = false;
					}
					else {
						...
						if (buttonClick) {
							...
							button = $( // type="button" so that it doesn't submit a form
								'<button type="button" class="' + classes.join(' ') + '"><span style="display:none">Icono</span>' +
									innerHtml +
								'</button>'
								)
								.click(function(ev) {
...
var basicDayGridMethods = {
	...
	renderHeadIntroHtml: function() {
		...
		if (view.weekNumbersVisible) {
			return '' +
				'<th scope="row" class="fc-week-number ' + view.widgetHeaderClass + '" ' + view.weekNumberStyleAttr() + '>' +
...
var agendaTimeGridMethods = {
	...
	renderHeadIntroHtml: function() {
		...
		if (view.opt('weekNumbers')) {
			...
			return '' +
				'<th scope="row" class="fc-axis fc-week-number ' + view.widgetHeaderClass + '" ' + view.axisStyleAttr() + '>' +
					'<span>' + // needed for matchCellWidths
						htmlEscape(weekText) +
					'</span>' +
				'</th>';
		    ...
		    return '<th scope="row" class="fc-axis ' + view.widgetHeaderClass + '" ' + view.axisStyleAttr() + '></th>';
		}
	},
