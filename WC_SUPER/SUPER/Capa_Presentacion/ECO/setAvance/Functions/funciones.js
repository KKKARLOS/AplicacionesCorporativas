...
function grabar(){
    try{
        ...
        mostrarProcesando();
        if ($I("txtAvanProd").value=="") $I("txtAvanProd").value="0,00";
...
var bGetDatosAvance = false;
function getDatosAvance(){
    try{
        if (bCambios) {
            jqConfirm("", "Datos modificados. ¿Deseas grabarlos?", "", "", "war", 320).then(function (answer) {
                if (answer) {
                    bGetDatosAvance = true;
                    grabar();
                    return;
                } else {
                    bCambios = false;
                    LLamarGetDatosAvance();
                }
            });
        } else LLamarGetDatosAvance();
    } catch (e) {
        mostrarErrorAplicacion("Error al ir a obtener los datos de los profesionales-1", e.message);
    }
}
function LLamarGetDatosAvance() {
    try {
        var js_args = "getDatosAvance@#@";
...
function LLamarInsertarmes() {
    try {
        ...
        var strEnlace = strServer + "Capa_Presentacion/ECO/getPeriodo.aspx?sDesde=" + nMesACrear + "&sHasta=" + nMesACrear + "&sAnomesMinimo=" + nMesACrear + "&sOpValidacion=1";
