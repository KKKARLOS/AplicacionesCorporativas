<%@ Page Language="C#" AutoEventWireup="true" CodeFile="getMonedaImportesProyecto.aspx.cs" Inherits="getMonedaImportesProyecto" %>
...
<base target="_self" />
    <title> ::: SUPER ::: - Cambio de la moneda de proyecto</title>
	...
	<!--
        ...
        function init(){
            try{
                ...
                //alert(opener.$I("hdnIdMoneda").value);
                ocultarProcesando();
            }catch(e){
        ...
        function setMonedaImportes(indexFila) {
            try {
                ...
                //alert("Antes: " + opener.$I("hdnIdMoneda").value + "\nAhora: "+ $I("tblDatos").rows[indexFila].id);
                if (opener.$I("hdnIdMoneda").value == $I("tblDatos").rows[indexFila].id) {
                    cerrarVentana();
                    return;
                }
                mostrarProcesando();
                var js_args = "setMonedaImportes@#@";
                js_args += opener.$I("hdnIdProyectoSubNodo").value + "@#@" +  $I("tblDatos").rows[indexFila].id;
        ...
        function aceptar() {
            try {
                var returnValue = $I("tblDatos").rows[nIndiceFila].id + "@#@" + $I("tblDatos").rows[nIndiceFila].cells[0].innerText;
                modalDialog.Close(window, returnValue);	
            } catch (e) {
                mostrarErrorAplicacion("Error seleccionar la fila", e.message);
            }
...
<table class="texto" style="margin-left:10px; width:320px;" cellpadding="5" >
    <tr>
        <td style="padding:10px; font-weight:bold; line-height:normal;">
            Atención: Un cambio de la moneda del proyecto desencadenará la modificación permanente de todos
            los importes del mismo. En caso de volver posteriormente a la moneda original, podría perderse
            precisión por cuestiones de redondeo.
        </td>
    </tr>
    <tr>
        <td>
