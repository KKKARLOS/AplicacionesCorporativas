...
public partial class Capa_Presentacion_ECO_ValorGanadoModal_Default : System.Web.UI.Page, ICallbackEventHandler
{
        ...
        if (!Page.IsCallback)
        {
            ...
            else
            {
                ...
                {//Vengo del gráfico de la consulta de proyectos con línea base
                    ...
                    txtNumPE.Text = idPE.ToString("#,###");
                    SqlDataReader dr = SUPER.Capa_Negocio.PROYECTO.GetDatosContratante(null, (int)Session["UsuarioActual"], idPE);
                    if (dr.Read())
                    {
                        idPSN = int.Parse(dr["t305_idproyectosubnodo"].ToString());
                        Session["ID_PROYECTOSUBNODO"] = idPSN.ToString();
                        Session["MODOLECTURA_PROYECTOSUBNODO"] = (bool)dr["modo_lectura"];
                        Session["RTPT_PROYECTOSUBNODO"] = (bool)dr["rtpt"];
                        Session["MONEDA_PROYECTOSUBNODO"] = dr["t422_idmoneda"].ToString();
                        lblMonedaImportes.InnerText = dr["t422_denominacion"].ToString();
                        txtDesPE.Text = dr["t301_denominacion"].ToString();
                        bDatosProyCargados = true;
                    }
                    dr.Close();
                    dr.Dispose();
            ...
            if (!Page.IsPostBack && Session["ID_PROYECTOSUBNODO"].ToString() != "")
            {
                ...
                if (!bDatosProyCargados)
                {
                    ...
                    try
                    {
                        dr = SUPER.Capa_Negocio.PROYECTO.GetDatosContratante(null, (int)Session["UsuarioActual"], oPSN.t301_idproyecto);
                        bool sw = false;
                        while (dr.Read())
                        {
                            if (!sw)
