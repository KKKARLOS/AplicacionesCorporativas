...
//        $I("procesando").style.left = 410;
        ...
        if (bHayPreferencia && $I("hdnInicio").value == "T" && $I("chkActuAuto").checked) {
            ...
            //Si estoy cargando porque se ha hecho submit no muestro la pestaña retráctil y recargo los criterios
...
function buscar(){
    try{
        ...
        //Guardo los criterios para recargarlos después del submit
        guardarCriterios();
...
function setAnno(sOpcion){
    try{
        ...
        mostrarErrorAplicacion("Error al establecer el año 2", e.message);
    }
}
function Limpiar()
{
...
function guardarCriterios() {
    try {
        ...
        sb.Append(getRadioButtonSelectedValue("rdbOperador", false) + "@#@"); //Operador lógico 4
