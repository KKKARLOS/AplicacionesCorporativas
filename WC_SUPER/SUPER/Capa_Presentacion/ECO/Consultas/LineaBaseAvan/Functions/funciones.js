...
//    //Dialogo para mostrar el gráfico
//    $("#divGrafico").dialog({
//        autoOpen: false,
//        resizable: false,
//        width: 560,
    ...
    if (aResul[1] != "OK"){
        ...
        switch (aResul[0]){
            case "buscar":
                ...
                aF = FilasDe("tblDatos");
                if (aF.length > 0) $I("btnGrafico").style.visibility = "visible";
                scrollTablaProy();
                window.focus();
                break;
...
function excel() {
    try {
        ...
        sb.Append("        <td style='width:auto; background-color: #BCD4DF; font-weight:bold;'>CPI</TD>");
        sb.Append("        <td style='width:auto; background-color: #BCD4DF; font-weight:bold;'>SPI</TD>");
        sb.Append("        <td style='width:auto; background-color: #BCD4DF; font-weight:bold;'>%Avance</TD>");
        sb.Append("        <td style='width:auto; background-color: #BCD4DF; font-weight:bold;'>CV</TD>");
        sb.Append("	</tr>");
        ...
        for (var i = 0; i < aFilas.length; i++) {
            ...
            sb.Append("<td>" + aFilas[i].cells[8].innerText + "</td>");

            dCPI = parseFloat(dfn(aFilas[i].cells[9].innerText));
            if (dCPI >= 0.95)
            ...
            sb.Append("<td style='" + sColorCPI + "'>" + dCPI + "</td>");

            dSPI = parseFloat(dfn(aFilas[i].cells[10].innerText));
            if (dSPI >= 0.95)
            ...
            sb.Append("<td style='" + sColorSPI + "'>" + dSPI + "</td>");

            sb.Append("<td>" + aFilas[i].cells[11].innerText + "</td>");
            sb.Append("<td>" + aFilas[i].cells[12].innerText + "</td>");
...
//Con la lista de proyectos de la vble de sesión, marcamos los checks de las filas correspondientes
function marcarProyectos() {
    try {
        var sAux = $I("hdnListaPE").value;
        if (sAux != "") {
            var o = JSON.parse(sAux); //convert the string into JSON object        
            $.each(o, function () {
                inner = this;
                for (i = 0; i < aF.length; i++) {
                    if (this.idPsn == getPSN(aF[i].getAttribute("id"))) {
        ...
        mostrarErrorAplicacion("Error al marcar Proyectos", e.message);
    }
}
function getPSN(sCad) {
    try {
        var aResul = sCad.split("///");
        return aResul[0];
    }
    catch (e) {
        mostrarErrorAplicacion("Error en getPSN", e.message);
    }
}
function borrarCatalogo(){
    try{
        if ($I("divCatalogo").children[0].innerHTML != ""){
            ...
            $I("btnGrafico").style.visibility = "hidden";
        }
	}catch(e){
                    ...
                    switch (nTipo) {
                        case 1:
                            ...
                            for (var i = 1; i < aElementos.length; i++) {
                                ...
                                oNF.cells[1].appendChild(oSpan);
                                oNF.cells[1].children[1].innerText = Utilidades.unescape(aDatos[2]);
                            }
                            $I("divAmbito").scrollTop = 0;
                            break;
    ...
    try {
        ...
        for (i = 0; i < aF.length; i++) {
            if (aF[i].cells[0].children[0].checked) {
                bHayPE = true;
                var sAux = aF[i].getAttribute("id");
                var aResul = sAux.split("///");
                var sIdPsn = aResul[0];
                var sIdPe = aF[i].cells[4].innerHTML.replace(".", "");
                var oProy = new Proy(sIdPe, sIdPsn, 0, 0);
                lstProyectos.push(oProy);
            }
        }
        var sUrl = "../../ValorGanado/Default.aspx?sp=" + codpar(idPSN);
        if (bHayPE) {
            var sl = JSON.stringify(lstProyectos);
            sUrl += "&lp=" + sl;
        }
        location.href = sUrl;
    } catch (e) {
...
var Proy = function (pe, psn, cpi, spi) {
    this.idPe = pe;
    this.idPsn = psn;
    this.cpi = cpi;
    this.spi = spi;
...
function mostrarGrafico() {
    ...
    try {
        if (!aF) {
            mmoff("War", "Debes marcar al menos un proyecto.", 300, 3000);
            return;
        }
        for (i = 0; i < aF.length; i++) {
            if (aF[i].cells[0].children[0].checked) {
                ...
                var dCpi = parseFloat(aF[i].getAttribute("cpi").replace(",","."));
                if (dCpi>=2)dCpi=1.98;
                
                var dSpi = parseFloat(aF[i].getAttribute("spi").replace(",", "."));
                if (dSpi >= 2) dSpi = 1.98;

                //var oProy = new Proy(sIdPe, sIdPsn, aF[i].getAttribute("cpi"), aF[i].getAttribute("spi"));
                var oProy = new Proy(sIdPe, sIdPsn, dCpi, dSpi);
                lstProyectos.push(oProy);
            }
        }
        if (bHayPE) {
            //Llamar a formulario con la lista de proyectos seleccionados
            //¿Quizá podría pasarse pares con el proyecto y el valor a representar en el gráfico?
            //En ese caso igual sería mejor mostrar una capa con el gráfico en vez de llamar a un formulario
            //$("#modal-content").toggleClass("active");
            //$("#divGrafico").dialog("open");
            var sl = JSON.stringify(lstProyectos);
            var sPantalla = strServer + "Capa_Presentacion/ECO/Consultas/LineaBaseAvan/Grafico.aspx?lp=" + sl + "&v=1";
