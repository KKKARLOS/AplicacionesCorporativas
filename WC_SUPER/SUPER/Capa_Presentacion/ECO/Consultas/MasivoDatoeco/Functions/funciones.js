...
function init(){
    try{
        ...
        $I("chkSectorCG").disabled = true;
        $I("chkSectorCF").disabled = true;
        $I("chkSegmentoCG").disabled = true;
        $I("chkSegmentoCF").disabled = true;

        $I("chkPaisCG").checked = true;
        $I("chkPaisCF").checked = false;
        $I("chkProvinciaCG").checked = true;
        $I("chkProvinciaCF").checked = false;

        $I("chkPaisCG").disabled = true;
        $I("chkPaisCF").disabled = true;
        $I("chkProvinciaCG").disabled = true;
        $I("chkProvinciaCF").disabled = true;

        //$I("chkSectorCG").disabled = true;
        switch ($I("hdnGrupoEco").value){
            case "1":
                $I("lblCLiFact").className = "label";
                $I("Img16").style.visibility = "hidden";
                $I("lblSociedad").className = "label";
                $I("Img21").style.visibility = "hidden";

                $I("fstPais").style.visibility = "hidden";
                $I("fstProvincia").style.visibility = "hidden";
                break;
            case "2":
                $I("lblCLiFact").className = "label";
                $I("Img16").style.visibility = "hidden";   
                ...
                break;         
            case "3":
                $I("lblCR").className = "label";
                $I("Img18").style.visibility = "hidden";
                $I("lblProveedor").className = "label";
                $I("Img20").style.visibility = "hidden";
                $I("chkSectorCG").disabled = false;
                $I("chkSectorCF").disabled = false;
                $I("chkSegmentoCG").disabled = false;
                $I("chkSegmentoCF").disabled = false;
                $I("chkPaisCG").disabled = false;
                $I("chkPaisCF").disabled = false;
                $I("chkProvinciaCG").disabled = false;
                $I("chkProvinciaCF").disabled = false;
                break;
        }        
	}catch(e){
...
function getCriterios(nTipo){
    try{
        ...
        var nTipoAux=nTipo;
        if (nTipo==17)nTipoAux=8;//Si es cliente de facturación llamo a la pantalla de clientes
        ...
        switch (nTipo){
            ...
            case 18://CR destino               
                ...
                break;                 
            case 19://Clases económicas
                strEnlace = strServer + "Capa_Presentacion/ECO/Consultas/getEstructuraEco/Default.aspx?nGrupo=" + $I("hdnGrupoEco").value;
                sTamano = sSize(950, 420);
                break;
            default:
                var nTipoAux = nTipo;
                if (nTipo == 17) nTipoAux = 8; //Si es cliente de facturación llamo a la pantalla de clientes
                if (nTipo == 21) nTipoAux = 22; //Sociedad que factura
                if (nTipo == 18) {
        ...
        switch (nTipo){
            ...
            case 19: oTabla = $I("tblClaseEco"); break;
            case 20: oTabla = $I("tblProveedores"); break;
            case 21: oTabla = $I("tblSociedad"); break;
        ...
        //                        oNF.cells[0].children[1].innerText = aDatos[2];
                            ...
                            break;                                       
                        case 19: 
        //                    $I("hdnGrupoEco").value = aElementos.shift();
        //                    $I("lblGrupoEco").innerText =" Grupo " + aElementos.shift();
                            BorrarFilasDe("tblClaseEco");
                            for (var i=0; i<aElementos.length; i++){
                                if (aElementos[i]=="") continue;
                                var aDatos = aElementos[i].split("@#@");
                                var oNF = $I("tblClaseEco").insertRow(-1);
                                oNF.id = aDatos[0]; 
                                oNF.style.height = "16px";
                                oNF.setAttribute("grupo", aDatos[1]);
                                oNF.setAttribute("subgrupo", aDatos[2]);
                                oNF.setAttribute("concepto", aDatos[3]);
                                oNF.insertCell(-1);
                                
        //                        oNF.cells[0].appendChild(document.createElement("<nobr class='NBR W260''></nobr>"));
        //                        oNF.cells[0].children[0].innerHTML = Utilidades.unescape(aDatos[4]);
                                oNF.cells[0].appendChild(oNobr.cloneNode(true), null);
                                oNF.cells[0].children[0].style.width = "260px";
                                oNF.cells[0].children[0].innerText = aDatos[4];
...
function setTodos() {
    try{
        ...
        setFilaTodos("tblClienteFact", (sOL=="1")?true:false, true);
        ...
        setFilaTodos("tblSociedad", (sOL == "1") ? true : false, true)

        //setFilaTodos("tblClaseEco", (sOL=="1")?true:false, true);
    ...
    try{
        ...
        switch (nTipo)
        {
            ...
            case 18: 
                    ...
                    js_ValNodos.length = 0;
                    break;            
            case 19: BorrarFilasDe("tblClaseEco"); break;
            case 20: BorrarFilasDe("tblProveedores"); break;
            case 21: BorrarFilasDe("tblSociedad"); break;
...
//                $I("chkExternos").checked = (aResul[51]=="1")? true:false;
                ...
                insertarFilasEnTablaDOM("tblClienteFact", aResul[48], 0);
                $I("divClienteFact").scrollTop = 0;
                
                BorrarFilasDe("tblClaseEco");
                insertarFilasEnTablaDOM("tblClaseEco", aResul[50], 0);
                $I("divClaseEco").scrollTop = 0;
                ...
                insertarFilasEnTablaDOM("tblSociedad", aResul[54], 0);
...
function Obtener(){
    try{
        ...
        if ($I("tblClaseEco").rows.length == 0){
            mmoff("Inf", "Debes seleccionar al menos una clase económica.", 320);
            return;
        }
...
//        else if ($I('rdbFormato_1').checked==true) 
            generarExcel();
	}catch(e){
...
function generarExcel(){
    try{
		...
		sb.Append($I("hdnGrupoEco").value + "@#@"); //Grupo economico -> tipo de informe 
        sb.Append(Utilidades.escape($I("lblGrupoEco").innerText) + "@#@"); //Descripcion Grupo economico
        ...
        sb.Append(getDatosTabla(17)+ "@#@"); //Clientes facturacion
        sb.Append(getDatosTabla(19)+ "@#@"); //Clases economicas
        //sb.Append(getDatosTabla(18)+ "@#@"); //CR'S destino
        sb.Append(js_nodos.join(",")+ "@#@"); //ids estructura CR'S destino
        sb.Append(getDatosTabla(20)+ "@#@"); //Proveedores
        sb.Append(getDatosTabla(21) + "@#@"); //Sociedades que facturan
        ...
        sb.Append((($I("chkPaisCG").checked) ? "1" : "0") + "@#@");
        sb.Append((($I("chkPaisCF").checked) ? "1" : "0") + "@#@");
        sb.Append((($I("chkProvinciaCG").checked) ? "1" : "0") + "@#@");
        sb.Append((($I("chkProvinciaCF").checked) ? "1" : "0") + "@#@");
        //sb.Append(location.href.substring(0,location.href.length-12),+"@#@");//URL para imágenes
    ...
    try{
        ...
        if (sb.ToString().length > 8000)
        {
            ...
            switch (nTipo)
            {
                ...
                case 17: mmoff("Inf", "Has seleccionado un número excesivo de clientes de facturación.", 500); break;
                ...
                case 21: mmoff("Inf", "Has seleccionado un número excesivo de sociedades.", 400); break;
...
function setPreferencia(){
    try{
        if ($I("tblClaseEco").rows.length == 0){
            mmoff("War", "Debes seleccionar al menos una clase económica.", 300);
            return;
        }
        document.forms["aspnetForm"].action=strAction;
        document.forms["aspnetForm"].target=strTarget;            
        ...
        sb.Append($I("hdnGrupoEco").value + "@#@");//Id Grupo Economico
        sb.Append("@#@");//Desc Grupo Economico Utilidades.escape($I("lblGrupoEco").innerText) + 
    ...
    try{
        jqConfirm("", "Pulsa ACEPTAR para confirmar la eliminación de todas las preferencias de esta pantalla.", "", "", "war", 450).then(function (answer) {
            if (answer) {
                ...
                var js_args = "delPreferencia@#@" + $I("hdnGrupoEco").value;
        ...
        modalDialog.Show(strServer + "Capa_Presentacion/getPreferencia.aspx?nP=" + codpar(sPant), self, sSize(450, 470))
	        .then(function(ret) {
	            if (ret != null) {
                    ...
                    js_args += ret + "@#@" + $I("hdnGrupoEco").value;
                    RealizarCallBack(js_args, "");
                    //borrarCatalogo();
	            }else ocultarProcesando();
	        }); 
...
function setResultadoOnline() {
    try {
        mostrarProcesando();
        var js_args = "setResultadoOnline@#@" + getRadioButtonSelectedValue("rdbResultadoCalculo", false);
