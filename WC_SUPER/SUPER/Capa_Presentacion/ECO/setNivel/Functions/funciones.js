...
function init(){
    try{
        ...
        colorearMeses();
        ocultarProcesando();
        if (bHayConsumos) mmoff("War", "Existen consumos por dedicaciones de profesionales", 400, 3500);
    }catch(e){
        ...
        switch (aResul[0]){
            case "grabar":
                ...
                if (bSalir) setTimeout("salir();", 50);
                    ...
                    if (bGetDatosNivel){
                        bGetDatosNivel = false;
                        setTimeout("getDatosNivel()", 20);
                    }
                    if (bInsertarMes){
                        bInsertarMes =  false;
            ...
            case "getDatosNivel":
                ...
                if (aResul[3]=="true" || aResul[4]=="true"){
                    bLectura = true;
                    setOp($I("btnGrabar"), 30);
                    setOp($I("btnGrabarSalir"), 30);
                }else{
                ...
                calcularTotal();
                if (aResul[4]=="true") mmoff("War", "Existen consumos por dedicaciones de profesionales", 400);
                break;
            case "getMesesProy":
    ...
    try{
        ...
        for (var i=0; i<aFila.length; i++){
            ...
            //if (aFila[i].unidades > 0){
                ...
                sb.Append(aFila[i].id +"##"); //ID nivel
                sb.Append(aFila[i].cells[2].innerText +"##"); //Coste
                sb.Append(fts(aFila[i].getAttribute("unidades")) +"///");//Unidades
                sw = 1;
            //}
        }
        if (sw == 0){
            mmoff("Inf", "No se han modificado los datos.", 240);
            ocultarProcesando();
            return;
        }
...
function comprobarDatos(){
    try{
        for (var i=0; i<aFila.length; i++){
            ...
            var nUnidades = parseFloat(dfn(aFila[i].cells[3].children[0].value));
            if (nUnidades < 0){
                ms(aFila[i]);
...
var bGetDatosNivel = false;
function getDatosNivel(){
    try{
        if (bCambios) {
            jqConfirm("", "Datos modificados. ¿Deseas grabarlos?", "", "", "war", 320).then(function (answer) {
                if (answer) {
        ...
        } else LLamarGetDatosNivel();
    }catch(e){
        mostrarErrorAplicacion("Error al ir a obtener los datos nivel-1", e.message);
    }
}
function LLamarGetDatosNivel() {
    try {
        $I("divCatalogo").children[0].innerHTML = "";
        iFila = -1;
        $I("lblTotalUnidades").innerText = "0,00";
        $I("lblTotalImporte").innerText = "0,00";

        var js_args = "getDatosNivel@#@";
		...
		mostrarErrorAplicacion("Error al ir a obtener los datos nivel-2", e.message);
    }
}
...
{
    try{
        getAuditoria(7, opener.$I("hdnIdProyectoSubNodo").value);
	}catch(e){
