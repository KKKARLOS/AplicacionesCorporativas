...
function comprobarDatos(){
    try{
        ...
        if (sw == 0){
            mmoff("War", "No hay tareas seleccionadas para asignar",290);
            return false;
        }
        ...
        if (idRecurso != ""){
            ...
            for (var i=0;i<aItems.length;i++){
                if (aItems[i].cells[1].children[0].checked){
                    ...
                    //sRes += aItems[i].id + "," + aItems[i].a + "," + aItems[i].nodo + "," + aItems[i].cierre + "," + aItems[i].psn;
                    sRes += aItems[i].id + "," + aItems[i].getAttribute("nodo") + "," + aItems[i].getAttribute("cierre") + "," + 
                            aItems[i].getAttribute("psn") + "," + aItems[i].getAttribute("deriva") + "," + aItems[i].getAttribute("notif") + "##";
                }
            }
            if (sw == 1) sRes = sRes.substring(0, sRes.length-2);
        }
...
function getProys() {
    try{
        ...
        var idRecurso = aFila[iFila].id;
        getItems(idRecurso);
    } 
    catch (e) {
        mostrarErrorAplicacion("Error al obtener proyectos del usuario", e.message);
    }
}
function getItems(idRecurso) {
    try{
        //var js_args = "getItems@#@" + idRecurso;
        var js_args = "getItems@#@" + idRecurso + "@#@" + (($I("chkSoloActivos2").checked) ? "1" : "0");      
