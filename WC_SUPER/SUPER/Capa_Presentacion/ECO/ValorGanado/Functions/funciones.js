...
function init(){
    try {
        ...
        //alert("ID Línea base: " + $I("hdnIdLineaBase").value + "\nMes referencia: " + $I("hdnMesReferencia").value);
        ToolTipBotonera("excel", "Exporta la información de las diferentes pestañas a Excel");
        ToolTipBotonera("eliminar", "Elimina la línea base en curso");
        iniciarPestanas();
...
function getDatosMes(nAnomes) {
    try {
        if (nAnomes != "190001") {
            ...
            $I("lblCodigo1").innerText = accounting.formatNumber(Math.abs(oVGActivo.codigo_1)) + " " + sMONEDA_VDP + " " + ((oVGActivo.codigo_1 < 0) ? "más" : "menos");
            ...
            $I("lblCodigo3").innerText = ((oVGActivo.codigo_3 >= 1) ? "retraso de " : "adelanto de ") + accounting.formatNumber(parseInt(Math.abs(oVGActivo.codigo_3), 10)) + " días";
