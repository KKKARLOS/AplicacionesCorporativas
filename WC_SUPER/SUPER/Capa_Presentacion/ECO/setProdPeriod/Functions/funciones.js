...
function init(){
    try{
        ...
        }else $I("txtConsPeriod").readOnly = false;
        ...
        switch (aResul[0]){
            case "grabar":
                ...
                if (bSalir) setTimeout("salir();", 50);
                    ...
                    if (bGetDatosPeriod){
                        bGetDatosPeriod = false;
                        setTimeout("getDatosPeriod()", 20);
                    }
                    if (bInsertarMes){
                        bInsertarMes =  false;
            ...
            case "getDatosPeriod":
                ...
                //alert(aResul[2]);
                $I("txtConsPeriod").value = aResul[2];
                
                if (aResul[3]=="true") $I("txtConsPeriod").readOnly = true;
                else $I("txtConsPeriod").readOnly = false;
...
function grabar(){
    try{
        ...
        sb.Append(($I("txtConsPeriod").value=="")? "0":$I("txtConsPeriod").value);
...
var bGetDatosPeriod = false;
function getDatosPeriod() {
    try {
        if (bCambios) {
            jqConfirm("", "Datos modificados. ¿Deseas grabarlos?", "", "", "war", 320).then(function (answer) {
                if (answer) {
                    bGetDatosPeriod = true;
                    grabar();
                    return;
                }
                else {
                    bCambios = false;
                    LlamarGetDatosPeriod();
                }
            });
        } else LlamarGetDatosPeriod();
    } catch (e) {
        mostrarErrorAplicacion("Error en  getDatosPeriod-1", e.message);
    }
}
function LlamarGetDatosPeriod() {
    try {
        var js_args = "getDatosPeriod@#@";
        ...
        mostrarErrorAplicacion("Error en  getDatosPeriod-2", e.message);
    }
}
