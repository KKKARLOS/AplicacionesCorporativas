...
public partial class Default : System.Web.UI.Page, ICallbackEventHandler
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!Page.IsCallback)
        {
            ...
            Master.TituloPagina = "Mantenimiento de niveles por " + Estructura.getDefLarga(Estructura.sTipoElem.NODO);
    ...
    private string obtenerTarifas(string sIdNodo)
    {
        try
        {
            ...
            SqlDataReader dr = COSTENIVEL.SelectByT303_idnodo(null, int.Parse(sIdNodo));
            ...
            while (dr.Read())
            {
                sb.Append("<tr id='" + dr["t380_idNivelConsumo"].ToString() + "' bd='' orden='" + dr["t380_orden"].ToString() + "' onclick='mm(event)' style='height:20px'>");
                ...
                sb.Append("<td><input type='text' class='txtL' style='width:260px' value=\"" + dr["t380_denominacion"].ToString() + "\" maxlength='30' onKeyUp='fm(event)'></td>");
                sb.Append("<td style='text-align:right; margin-right:5px;'><input type='text' class='txtNumL' style='width:90px;' value=\"" + decimal.Parse(dr["t380_costenivelH"].ToString()).ToString("N") + "\" onKeyUp='fm(event)' onfocus='fn(this)'></td>");
                sb.Append("<td style='text-align:right; margin-right:5px;'><input type='text' class='txtNumL' style='width:90px;' value=\"" + decimal.Parse(dr["t380_costenivelJ"].ToString()).ToString("N") + "\" onKeyUp='fm(event)' onfocus='fn(this)'></td>");
                sb.Append("</tr>");
            }
            dr.Close();
            dr.Dispose();
    ...
    protected string Grabar(string strDatos)
    {
        ...
        try
        {
            ...
            foreach (string oTarifa in aTarifa)
            {
                ...
                COSTENIVEL.Delete(tr, int.Parse(aValores[1]));
            } 
            ...
            foreach (string oTarifa in aTarifa)
            {
                ...
                {
                    case "I":
                        nAux = COSTENIVEL.Insert(tr,    int.Parse(aValores[6]), 
                                                        Utilidades.unescape(aValores[2]), 
                                                        decimal.Parse(aValores[3]), 
                                                        decimal.Parse(aValores[4]),
                                                        byte.Parse(aValores[5]));
                        if (sElementosInsertados == "") sElementosInsertados = nAux.ToString();
                        else sElementosInsertados += "//" + nAux.ToString();
                        break;
                    case "U":
                        COSTENIVEL.Update(tr, int.Parse(aValores[1]),
                                            int.Parse(aValores[6]),
                                            Utilidades.unescape(aValores[2]), 
                                            decimal.Parse(aValores[3]),
                                            decimal.Parse(aValores[4]),
                                            byte.Parse(aValores[5]));
