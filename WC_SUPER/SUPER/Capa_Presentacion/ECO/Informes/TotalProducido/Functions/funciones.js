...
function init(){
    try{
        ...
        if (bHayPreferencia){
            aProduccion = $I("hdnProducPref").value.split("///");
            var aDatos;
            for (var x = 0; x < aProduccion.length; x++) {
                if (aProduccion[x] != "") {
                    aDatos = aProduccion[x].split("##");
...
function mTabla(sTipo, sOp)
{
    try
    {
        var aInput = $I('tblProduccion').getElementsByTagName("INPUT");
        //var aInput = document.getElementsByTagName("INPUT");
	    for (i=0;i<aInput.length;i++){
	        if (aInput[i].type != "checkbox") continue;
	        if (aInput[i].getAttribute("t") == sTipo) {
	            if (sOp == 1) {
...
function setTodos() {
    try{
        ...
        //$I("chkProfesionales").checked =  true;
        //mTabla(1);
        ...
        switch (aResul[0]){
           case "generarExcel":
            ...
            case "getPreferencia":
                ...
                if (aResul[6]=="1") $I("rdbFormato_0").checked = true;
                else $I("rdbFormato_1").checked = true;   
 
                $I("chkProfesionales").checked = (aResul[7]=="1")? true:false;

                aProduccion = aResul[44].split("///");
                var aDatos;
                for (var x=0; x<aProduccion.length; x++){
                    aDatos = aProduccion[x].split("##");
                    $I(aDatos[0]).checked = (aDatos[1]=="1")? true:false;
                ...
                else $I("rdbOperador_1").checked = false;
...
function Obtener(){
    try{
        ...
        if ($I("cboConceptoEje").value == ""){
            mmoff("Inf", "El concepto eje es obligatorio.", 220);
            return;
        } 
        var sPro = 0;
        
        if  ($I("chkProfesionales").checked) sPro = 1;
	    ...
	    if (sPro==0)
	    {
	         mmoff("Inf", "Debes indicar la base de calculo", 280);
	         return;
	    }
		...
		mostrarErrorAplicacion("Error al obtener.", e.message);
    }
}
        ...
        if (parseInt($I("cboConceptoEje").value, 10) >= 7){
            ...
            if ($I("tblAmbito").rows.length > 0 && $I("tblAmbito").rows[0].id != "-999"){
                for (var i=0; i < $I("tblAmbito").rows.length; i++){
                    if (parseInt($I("tblAmbito").rows[i].getAttribute("tipo"), 10) < nOpcion) nOpcion = parseInt($I("tblAmbito").rows[i].getAttribute("tipo"), 10);
                }
...
//        sb.Append(nNivelEstructura+"@#@"); //sNivelEstructura 
        ...
        var sValores="";
        if ($I("chkProfesionales").checked) sValores = $I('hdnIdsCptosProfesionales').value;
	    ...
	    {
	        if (aInput[i].type != "checkbox") continue;
	        if (aInput[i].checked) sValores += aInput[i].id + ",";
	    }
	    
        if (sValores.length>0) sValores=sValores.substring(0,sValores.length-1);
        sb.Append(sValores+ "@#@"); //Valores produccion
...
//		$I('hdnNivelEstructura').value = nNivelEstructura;
        ...
        $I('hdnProduccion').value="";
        if ($I("chkProfesionales").checked) $I('hdnProduccion').value = $I('hdnIdsCptosProfesionales').value;
	    ...
	    {
	        if (aInput[i].type != "checkbox") continue;		    
            if (aInput[i].checked) $I('hdnProduccion').value+=aInput[i].id+",";
	    }
	    
        if ($I('hdnProduccion').value.length>0) $I('hdnProduccion').value=$I('hdnProduccion').value.substring(0,$I('hdnProduccion').value.length-1);
        ...
        //*SSRS
		sConcepto = "";
		if ($I("cboConceptoEje").value == "0") sConcepto = "Estructura organizativa";
		...
		var params = {
		    reportName: "/SUPER/sup_produccion",
		    tipo: "PDF",
		    ...
		    sProduccion: $I("hdnProduccion").value,
...
function setPreferencia(){
    try{
        ...
        sb.Append(($I("chkProfesionales").checked)? "1@#@":"0@#@");
...
function getValoresMultiples(){
    try{
	    ...
	    for (i=0;i<aInput.length;i++)
	    {
	        if (aInput[i].type != "checkbox") continue;
	        if (sb.buffer.length>0) sb.Append("///");
		    sb.Append("15##"+ aInput[i].id +"##");
		    sb.Append((aInput[i].checked) ? "1" : "0");
	    }
        return sb.ToString();
    }catch(e){
		mostrarErrorAplicacion("Error al obtener los IDs de los criterios.", e.message);
...
function Limpiar()
{
    ...
    for (var i=0; i<aTable.length; i++){
        ...
        if (aTable[i].id == "tblProduccion") continue;        
