        ...
        switch (aResul[0]){
           case "generarExcel":
            ...
            case "getPreferencia":
                ...
                if (aResul[52]=="1") $I("rdbFormato_0").checked = true;
...
//        sb.Append(nNivelEstructura+"@#@"); //sNivelEstructura 
		...
		sb.Append(getRadioButtonSelectedValue("rdbOrdenacion", false)+ "@#@"); //Ordenación Proyecto-Recurso / Recurso-Proyecto
		sb.Append(getRadioButtonSelectedValue("rdbConsumos", false)+ "@#@"); // Profesionales todos o con consumos 

        var sSeleccion="";
        if ($I("chkPropios").checked) sSeleccion+="P,";
        if ($I("chkOtros").checked) sSeleccion+="O,";
        if ($I("chkExternos").checked) sSeleccion+="E,";
        sSeleccion=sSeleccion.substring(0,sSeleccion.length-1);
        sb.Append(sSeleccion+ "@#@"); //seleccion tipo recurso
...
function Exportar(strFormato){
    try{     
        if (!($I("chkPropios").checked)&&!($I("chkOtros").checked)&&!($I("chkExternos").checked))        
        {
            mmoff("Inf", "Debes seleccionar algún tipo de profesional.",330);  
            return;      
        }
...
//		$I('hdnNivelEstructura').value = nNivelEstructura;
		...
		$I('hdnSeleccion').value = sSeleccion;
		...
		if (getRadioButtonSelectedValue("rdbOrdenacion") == "1") CABECERA = "ASIGNACIÓN DE PROFESIONALES A PROYECTOS";
		else CABECERA = "ASIGNACIÓN DE PROYECTOS A PROFESIONALES";

		nDesde = ($I("hdnDesde").value).substring(0, 4) + "-" + ($I("hdnDesde").value).substring(4, 6) + "-01";
		var hastaDate = new Date(($I("hdnHasta").value).substring(0, 4), ($I("hdnHasta").value).substring(4, 6), 0);
		strFecha = hastaDate.getFullYear() + "-" + (hastaDate.getMonth()+1) + "-" + hastaDate.getDate();

		if (getRadioButtonSelectedValue("rdbOperador") == 1) rdboperador = true;
		else rdboperador = false;

		if (getRadioButtonSelectedValue("rdbOrdenacion") == 1) rdbOrdenacion = true;
		else rdbOrdenacion = false;

		if (getRadioButtonSelectedValue("rdbConsumos") == 1) rdbConsumos = true;
		else rdbConsumos = false;
		...
		var params = {
		    ...
		    cOrdenacion: rdbOrdenacion,
		    cConSinconsumos: rdbConsumos,
		    cTipoProfesional: $I("hdnSeleccion").value,
		    sCNP: $I("hdnCNP").value,
		...
		if (getRadioButtonSelectedValue("rdbOrdenacion") == "1") params["reportName"] = "/SUPER/sup_asignproyprof";
		else params["reportName"] = "/SUPER/sup_asignprofproy";
