...
function aceptar(){
    try{
        ...
        if (strDatos != "")
            ...
            ocultarProcesando();
            mmoff("Inf", "Debes seleccionar algún profesional", 230);
            return;
        }
...
function insertarRecurso(oFila, nOp){
    try{
        ...
        if (nOp == 1){
            for (var i=0; i < tblDatos2.rows.length; i++){
                aProfesionales[aProfesionales.length] = tblDatos2.rows[i].id;
            }
        }
...
function insertarRecurso2(idRecurso, nombre, idCal, desCal, sexo, nOp) {
    try {
        var bExiste = false;
        ...
        NewRow.id = idRecurso;
        NewRow.setAttribute("idCal", idCal);
        NewRow.setAttribute("descal", desCal);
        NewRow.setAttribute("nodo", "-2");
        NewRow.setAttribute("baja", "0");
        NewRow.setAttribute("costecon", "0");
        NewRow.setAttribute("costerep", "0");
        NewRow.setAttribute("sexo", sexo);
        NewRow.setAttribute("tipo", "F");

        var dtHoy = new Date();
        var mes = dtHoy.getMonth() + 1;
        var sFecha = ('0'+dtHoy.getDate()).slice(-2).toString() + '/' + ('0'+mes).slice(-2).toString() + '/' + dtHoy.getFullYear().toString();
        NewRow.setAttribute("alta", sFecha);

        var oNC2 = NewRow.insertCell(-1);
        if (sexo == "V") {
            oNC2.appendChild(oImgFV.cloneNode(true), null);
        } 
        else {
            oNC2.appendChild(oImgFM.cloneNode(true), null);
        }
        var oNoBr = document.createElement("nobr");
        oNoBr.className = "NBR";
        oNoBr.style.width = "190px";
        oNC2.appendChild(oNoBr.cloneNode(true));
        NewRow.cells[0].children[1].innerText = nombre;
...
function addForaneo() {
    try {
        mostrarProcesando();
        var strEnlace = strServer + "Capa_Presentacion/ECO/Foraneo/Nuevo/Default.aspx";
        //var ret = window.showModalDialog(strEnlace, self, sSize(540, 300));
        modalDialog.Show(strEnlace, self, sSize(540, 300))
	        .then(function(ret) {
                if (ret != null) {
                    var aDatos = ret.split("@#@");
                    //insertarRecurso2(idRecurso, nombre, idCal, desCal, sexo, 1)
                    if (aDatos[0]=="T")
                        insertarRecurso2(aDatos[1], aDatos[2], aDatos[3], aDatos[4], aDatos[5], 1)
                }
                ocultarProcesando();
	        });
    } catch (e) {
        mostrarErrorAplicacion("Error al añadir foráneos", e.message);
    }
}
