...
var oIFrameCal = this.parent.document.getElementById("iFrameCalculadora");
var bCalculo = false;
...
function keyboard(evt)
{
    ...
    if (key == 120) hide_calc();   // F9   --> Cerrar calculadora
    if (key == 121) llevar();   // F10  --> "LLevar y cerrar"
    //alert(key);
...
function getCookie(name)
{
    ...
    return Utilidades.unescape(dc.substring(begin + prefix.length, end));
}
...
{
    try{
        var oDestino = this.parent.oDestinoCalculadora;
        if (typeof(oDestino) != "undefined" && oDestino != ""){
            calc();
            this.parent.document.getElementById(this.parent.oDestinoCalculadora).value = document.calculator.expr.value.replace(".", ",").ToString("N");
            this.parent.document.getElementById(this.parent.oDestinoCalculadora).style.backgroundImage = "";
            this.parent.document.getElementById(this.parent.oDestinoCalculadora).style.backgroundRepeat = "";
            this.parent.document.getElementById(this.parent.oDestinoCalculadora).focus();
            try { this.parent.document.getElementById(this.parent.oDestinoCalculadora).fireEvent("onchange"); } catch (e) { }	        
        }
        hide_calc();
    }catch(e){
...
var nOpacidadCalculadora = 0;
var nTimeOutMostrarCalculadora;
function show_calc()
{
    try{
        //window.top.document.getElementById("iFrameCalculadora").style.visibility = "visible";
        this.parent.document.getElementById("iFrameCalculadora").style.visibility = "visible";
        setOpacityCalc();
...
function setOpacityCalc()
{
    try{
        nOpacidadCalculadora += 5;
        this.parent.document.getElementById("iFrameCalculadora").style.filter = "progid:DXImageTransform.Microsoft.Alpha(Opacity=" + nOpacidadCalculadora + ")";
        this.parent.document.getElementById("iFrameCalculadora").style.opacity = parseInt((nOpacidadCalculadora / 100), 10);
        if (nOpacidadCalculadora < 100)
            nTimeOutMostrarCalculadora = setTimeout("setOpacityCalc();", 50);
        else
            clearTimeout(nTimeOutMostrarCalculadora);
    }catch(e){
	    mostrarErrorAplicacion("Error al dar opacidad la calculadora", e.message);
    }
}
...
function hide_calc()
{
    try{
        ...
        if (typeof(oDestino) != "undefined" && oDestino != ""){
            this.parent.document.getElementById(this.parent.oDestinoCalculadora).style.backgroundImage = "";
            this.parent.document.getElementById(this.parent.oDestinoCalculadora).style.backgroundRepeat = "";
            this.parent.oDestinoCalculadora = "";
        }
        clear_mem();
        delOpacityCalc();
    }catch(e){
...
function delOpacityCalc()
{
    try{
        nOpacidadCalculadora -= 5;
        this.parent.document.getElementById("iFrameCalculadora").style.filter = "progid:DXImageTransform.Microsoft.Alpha(Opacity=" + nOpacidadCalculadora + ")";
        this.parent.document.getElementById("iFrameCalculadora").style.opacity = parseInt((nOpacidadCalculadora / 100), 10);
        if (nOpacidadCalculadora > 0)
            nTimeOutMostrarCalculadora = setTimeout("delOpacityCalc();", 50);
        else{
            clearTimeout(nTimeOutMostrarCalculadora);
            this.parent.document.getElementById("iFrameCalculadora").style.visibility = "hidden";
        }
    }catch(e){
	    mostrarErrorAplicacion("Error al restar opacidad la calculadora", e.message);
    }
}
...
//        
	    if (document.all) {
	        popupCalculadoraofsX = e.clientX;
	        popupCalculadoraofsY = e.clientY;
	    }
	    else {
	        popupCalculadoraofsX = e.pageX;
	        popupCalculadoraofsY = e.pageY;
	    }

	    if (typeof document.attachEvent != 'undefined') {
	        window.document.attachEvent("onmousemove", popupCalculadoraespopup_DragDropMove);
	        window.document.attachEvent("onmouseup", popupCalculadoraespopup_DragDropStop);
	    } else {
	        document.body.addEventListener("mousemove", popupCalculadoraespopup_DragDropMove, false);
	        document.body.addEventListener("mouseup", popupCalculadoraespopup_DragDropStop, false);
	    }      	  
//	      
...
////		
	...
	{
	    //el=this.parent.document.getElementById("iFrameCalculadora");
		//el=oIFrameCal;
	    if (!e) e = event; 
		if (document.all)
		{
            if (typeof(window.dialogArguments)=="undefined"){ //Página maestra
		        oIFrameCal.style.left = oIFrameCal.offsetLeft + e.clientX - popupCalculadoraofsX;
                oIFrameCal.style.top = Math.max(122, oIFrameCal.offsetTop + e.clientY - popupCalculadoraofsY);
			    //oIFrameCal.style.top = oIFrameCal.offsetTop + event.clientY - popupCalculadoraofsY;
            }else{ //Página modal
			    oIFrameCal.style.left = oIFrameCal.offsetLeft + e.clientX - popupCalculadoraofsX;
			    oIFrameCal.style.top = oIFrameCal.offsetTop + e.clientY - popupCalculadoraofsY;
            }
            e.cancelBubble = true;
            if (e.stopPropagation) e.stopPropagation();
		}
		else {
		    oIFrameCal.style.left = oIFrameCal.offsetLeft + e.pageX - popupCalculadoraofsX + 'px';
		    oIFrameCal.style.top = oIFrameCal.offsetTop + e.pageY - popupCalculadoraofsY + 'px';
		
//		    oIFrameCal.style.left = (e.pageX * 1 - popupCalculadoraofsX) + 'px';
//		    oIFrameCal.style.top = (e.pageY * 1 - popupCalculadoraofsY) + 'px';
			e.cancelBubble = true;
			if (e.stopPropagation) e.stopPropagation();
		}

		//if ((e.button)==0) popupCalculadoraespopup_DragDropStop();
	}

	function popupCalculadoraespopup_DragDropStop() {

	    if (typeof document.detachEvent != 'undefined') {
	        window.document.detachEvent("onmousemove", popupCalculadoraespopup_DragDropMove);
	        window.document.detachEvent("onmouseup", popupCalculadoraespopup_DragDropStop);
	    } else {
	        document.body.removeEventListener("mousemove", popupCalculadoraespopup_DragDropMove, false);
	        document.body.removeEventListener("mouseup", popupCalculadoraespopup_DragDropStop, false);
	    }      	  
