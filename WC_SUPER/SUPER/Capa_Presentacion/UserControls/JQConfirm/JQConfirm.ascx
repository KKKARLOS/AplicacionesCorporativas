...
<script type="text/javascript">
    ...
    //botonCancelarFoco: En caso de que el parámetro sea true, se asigna el foco al segundo botón del dialog
    function jqConfirm(titulo, mensaje, botonAceptarTexto, botonCancelarTexto, tipoDialogo, ancho, botonCancelarFoco) {

        //Si no se indica el tipo de dialogo por defecto se asigna el dialogo question
        if (titulo == "" || !titulo) titulo = "<div style='font-size: 13px;font-family: Arial, Helvetica, sans-serif;'>Confirmación</div>";
        var isClicked = false;
        //Si no se indica el tipo de dialogo por defecto se asigna el dialogo question
        ...
        var opt = {
            ...
            width: ancho,           
            buttons: botones,
            open: function () { if (botonCancelarFoco) $('.ui-dialog-buttonset button:eq(1)').focus(); },
