...
function init() {
    try {
        ...
        sDescripcion = $I("txtDesActividad").value;
        ...
        if ($I("hdnNivelPresupuesto").value == "A") {
...
function aceptar() {
    ...
    strRetorno += "@#@A@#@" + sDescripcion + "@#@" + sFIV + "@#@" + sFFV + "@#@" + bFechaModificada + "@#@" + sPresupuesto;
...
function vigenciaModificada() {
    ...
    if (bFechaModificada) {
        ...
        sRes += "las tareas que dependen de esta actividad.<br /><br />";
...
function grabar() {
    try {
        ...
        mostrarErrorAplicacion("Error al grabar los datos de la actividad", e.message);
    }
}
function LLamarGrabar() {
    var sOPcionBD, bCambioRecursos = false, bCambioRTPT = false, sAux;
    try {
        ...
        var js_args = "grabar@#@" + $I("hdnIDActividad").value + "##" + $I("hdnIDFase").value + "##" + $I("hdnCRActual").value +
...
function grabarP0() {
    ...
    if (aPestGral[0].bModif) {
        js_args += Utilidades.escape($I("txtDesActividad").value) + "##"; //0
...
function grabarP1() {
    ...
    if (aPestGral[1].bModif) {
        if (aPestProf[0].bModif) {
            //Control de los tecnicos asociados a la actividad
            //Primero recojo los que ya pertencían a la actividad y ahora se Asignan/Desasignan completamente
            ...
            for (var i = 0; i < aRecursos.length; i++) {
                ...
                if (aRecursos[i].opcionBD != "") {
                    ...
                    js_args += aRecursos[i].indicaciones + "##"; //Ya viene escapeado en el array aRecursos
        ...
        if (aPestProf[1].bModif) {
            //Control de los Grupos Funcionales asociados al pool de la actividad
            ...
            js_args += "@#@";
            //Control de los profesionales asociados al pool de la actividad
            var aPoolProf = FilasDe("tblAsignados3");
            for (var i = 0; i < aPoolProf.length; i++) {
                if (aPoolProf[i].getAttribute("bd") != "N") {
...
function comprobarDatos() {
    try {
        if ($I("txtDesActividad").value == "") {
            tsPestanas.setSelectedIndex(0);
            mmoff("War", "Debes indicar el nombre de la actividad", 280);
            return false;
        }
        ...
        if (objRec == null) { //No existe en el array
            ...
            for (var i = 0; i < aFila.length; i++) {
                if (aFila[i].id == idRecurso) {
                    ...
                    //alert("El técnico indicado ya se encuentra asignado a la actividad");
                    bExiste = true;
                    break;
                }
            }
            if (bExiste) {
                ...
                insertarRecursoEnArray("I", $I("hdnIDActividad").value, idRecurso, oFila.innerText, "", "", oFila.getAttribute("idTarifa"), "1", "", "0");
...
//***************************************************************************************
//Funciones de los botones asociados a la lista de técnicos asignados a la actividad
//***************************************************************************************
...
function mostrarTareasRecurso(idRecurso) {
    try {
        //alert("IDPT: "+$I("hdnIDActividad").value + "   idRecurso: "+ idRecurso);
        mostrarProcesando();
    ...
    try {
        ...
        if (sSistemaPestanas == "tsPestanas" || sSistemaPestanas == "ctl00_CPHC_tsPestanas") {
            if (nPestanaPulsada > 0) {
                ...
                if ($I("txtIdActividad").value == "0" || $I("txtIdActividad").value == "") {
                    mmoff("Inf", "El acceso a la pestaña seleccionada, requiere seleccionar una actividad.", 500);
                    eventInfo.cancel();
                    return false;
                }
            }
...
function getDatos(iPestana) {
    try {
        mostrarProcesando();
        var js_args = "getDatosPestana@#@" + iPestana + "@#@" + $I("txtIdActividad").value + "@#@" + $I("hdnCRActual").value;
...
function getDatosProf(iPestana) {
    try {
        mostrarProcesando();
        RealizarCallBack("getDatosPestanaProf@#@" + iPestana + "@#@" + $I("hdnIDPT").value + "@#@" + $I("hdnIDFase").value + "@#@" + $I("txtIdActividad").value + "@#@" + $I("hdnCRActual").value, "");
                ...
                case "divAsignados":
                    ...
                    if (nOpcionDD == 1) {
                        ...
                        if (sw == 0) {
                            ...
                            insertarRecursoEnArray("I", $I("hdnIDActividad").value, oRow.id, oRow.innerText, "", "", oRow.getAttribute("idTarifa"), "1", "", "0");
    ...
    try {
        ...
        for (var i = nFilaVisible; i < nUltFila; i++) {
            ...
            if (!tblAsignados.rows[i].getAttribute("sw")) {
                ...
                if (oFila.getAttribute("sexo") == "V") {
                    switch (oFila.getAttribute("tipo")) {
                        case "E": oFila.cells[2].appendChild(oImgEV.cloneNode(true), null); break;
                        case "N": oFila.cells[2].appendChild(oImgNV.cloneNode(true), null); break;
                        case "P": oFila.cells[2].appendChild(oImgPV.cloneNode(true), null); break;
                        case "F": oFila.cells[2].appendChild(oImgFV.cloneNode(true), null); break;
                    }
                } else {
                    switch (oFila.getAttribute("tipo")) {
                        case "E": oFila.cells[2].appendChild(oImgEM.cloneNode(true), null); break;
                        case "N": oFila.cells[2].appendChild(oImgNM.cloneNode(true), null); break;
                        case "P": oFila.cells[2].appendChild(oImgPM.cloneNode(true), null); break;
                        case "F": oFila.cells[2].appendChild(oImgFM.cloneNode(true), null); break;
                    }
                }
                if (oFila.getAttribute("baja") == "1")
...
function getRecursos() {
    try {
        ...
        RealizarCallBack("getRecursos@#@" + $I("txtIdActividad").value + "@#@" + $I("hdnCRActual").value + "@#@" + sVer, "");
...
function getRecursosPool() {
    try {
        ...
        RealizarCallBack("getRecursosPool@#@" + $I("hdnIDPT").value + "@#@" + $I("hdnIDFase").value + "@#@" + $I("txtIdActividad").value + "@#@" + $I("hdnCRActual").value + "@#@" + sVer, "");
...
function activarOTC(bHeredada) {
    try {
        ...
        mostrarErrorAplicacion("Error al activar la OTC de la actividad", e.message);
    }
}
function nuevoDoc1() {
    var sIdActiv = $I('hdnIDActividad').value;

    if ((sIdActiv == "") || (sIdActiv == "0")) {
        mmoff("Inf", "La actividad debe estar grabada para poder asociarle documentación", 400);
    }
    else {
        nuevoDoc('A', sIdActiv);
    }
...
function verCodigo() {
    mmoff("InfPer", "El código de la actividad es: " + $I("txtIdActividad").value, 260);
}