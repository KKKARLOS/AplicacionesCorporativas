...
function init(){
    try{
        ...
        if ($I("hdnAcceso").value=="R"){
            ...
            //$I("lblProy").setAttribute("className", "texto");
...
function grabar() {
    try {
        ...
        js_args += Utilidades.escape($I("hdnDesPE").value) + "##"; //12
        ...
        var aTareas = $I("tblTareas").getElementsByTagName("TR");
        for (i = 0; i < aTareas.length; i++) {
            sAux = aTareas[i].getAttribute("bd");
            if (sAux == "I" || sAux == "D") {
...
function convocar(idUsuario, strUsuario, sMail){
    try{
	    ...
	    oNC1 = oNF.insertCell(-1);
	    oNC1.style.width = "365px";
	    oNC1.innerText = strUsuario;
	    
	    oNC2 = oNF.insertCell(-1);
	    oNC2.setAttribute("style", "width:40px;");
...
function tareas() {
    try {
        var aOpciones, iCodTarea, sDesTarea, sCodCR, sCad, sETPL, sFIPL, sFFPL, sETPR, sFFPR, sConsumo, sAvance;
        if (getOp($I("btnAddTarea")) == 30) return;

        //sCodProyEco=$I("hdnIdPE").value;
        var strEnlace = strServer + "Capa_Presentacion/PSP/Hito/obtenerTarea.aspx?nPE=" + $I("hdnT305IdProy").value;
        mostrarProcesando();
...
function borrarTareas() {
    try {
        ...
        if (iFilas == 0) mmoff("Inf","Para eliminar una tarea asociada a la acción debe seleccionar\nla fila correspondiente",400);
        else activarGrabar();
    } catch (e) {
            ...
            switch (oTarget.id) {
		        ...
		        case "ctl00_CPHC_divCatalogo2":
                    ...
                    if (sw == 0){
                        ...
                        oCloneNode.setAttribute("className", "");
