...
function grabar(){
    try{
        ...
        js_args += Utilidades.escape($I("hdnT305IdProy").value) +"@#@"; //28
            ...
            for (i=0;i<aResul.length;i++){
                if (aResul[i] != ""){
                    if (!validarEmail(fTrim(aResul[i]))){
                        tsPestanas.setSelectedIndex(0);
                        mmoff("War", "La dirección de correo indicada en el campo 'Notificar a' no es válida (" + aResul[i] + ")", 400);
                        return false;
                    }
                }
            }
