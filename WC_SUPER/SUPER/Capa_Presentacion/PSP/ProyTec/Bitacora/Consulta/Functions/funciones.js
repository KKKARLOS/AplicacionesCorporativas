...
function obtenerPTs(){
    try{
	    var aOpciones, idPE, sPE, strEnlace,sAncho,sAlto;
	    ...
	    modalDialog.Show(strEnlace, self, sTamano)
            .then(function(ret) {
	            if (ret != null) {
	                aOpciones = ret.split("@#@");
	                $I("txtIdPT").value = aOpciones[0];
	                $I("txtDesPT").value = aOpciones[1];
	                if ($I("hdnT305IdProy").value == "") {
	                    //if (idPE == "" || idPE == "0") {
	                    $I("txtCodProy").value = aOpciones[2];
	                    $I("txtNomProy").value = aOpciones[3];
	                    $I("hdnT305IdProy").value = aOpciones[4];
	                    $I("txtEstado").value = aOpciones[5];
	                    $I("txtUne").value = aOpciones[6];
	                    //}
	                }
