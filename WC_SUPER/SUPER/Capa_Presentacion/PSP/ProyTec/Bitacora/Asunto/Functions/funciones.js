...
function grabar(){
    try{
        ...
        js_args += "##"; //17 Utilidades.escape($I("hdnCRActual").value) +
        ...
        js_args += Utilidades.escape($I("hdnDesPE").value) +"##"; //27
        js_args += Utilidades.escape($I("hdnIdPT").value) +"##"; //28
        js_args += Utilidades.escape($I("hdnDesPT").value) +"@#@"; //29
...
function comprobarDatos(){
    try{
        ...
        if ($I("cboEstado").value=="0"){
            if (sIdAsunto!="" && sIdAsunto!="-1"){
                tsPestanas.setSelectedIndex(0);
                mmoff("War", "Debes indicar el estado del asunto", 250);
                return false;
            }
        }
        if ($I("cboTipo").selectedIndex == -1){
            tsPestanas.setSelectedIndex(0);
            mmoff("War", "Debes indicar el tipo de asunto", 230);
            return false;
        }
        //Alertas por e-mail
...
function convocar(idUsuario, strUsuario, sMail) {
    try{
        ...
        oNC3.style.width = "390px";
        oNC3.innerText = strUsuario;

        //oNC2.appendChild(document.createElement("<input type='checkbox' checked='true' style='width:15' id='chkEst" + oNF.rowIndex + "' class='checkTabla' onclick='actualizarDatos(this);'>"));
        oNC4 = oNF.insertCell(-1);
        oNC4.style.width = "20px";
