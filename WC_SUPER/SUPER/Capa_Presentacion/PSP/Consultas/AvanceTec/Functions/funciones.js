...
function moverScroll(e){
    try {
        ...
        oDivMonedaImportes.style.left = (408 - oElement.scrollLeft) +"px";
    } catch (e) {
...
function mdpsn(oFila){
    try{
	    ...
	    strEnlace += "&rtpt=" + oFila.getAttribute("rtpt");
	    strEnlace += "&origen=T";
...
function excel(){
    try {
        ...
        sb.Append("        <td width='260px' colspan='4' style='background-color: #E4EFF3;'>Planificado</TD>");
        sb.Append("        <td width='325px' colspan='5' style='background-color: #E4EFF3;'>IAP</TD>");
        sb.Append("        <td width='240px' colspan='4' style='background-color: #E4EFF3;'>Previsto</TD>");
        sb.Append("        <td width='110px' colspan='2' style='background-color: #E4EFF3;'>Avance</TD>");
        sb.Append("        <td width='130px' colspan='3' style='background-color: #E4EFF3;'>Indicadores</TD>");
		sb.Append("	</TR>");
		sb.Append("	<TR align=center>");
        sb.Append("        <td style='width=:70px; background-color: #BCD4DF;'>Categoría</TD>");
        sb.Append("        <td style='width=:160px; background-color: #BCD4DF;'>Cualidad</TD>");
        sb.Append("        <td style='width=:70px; background-color: #BCD4DF;'>Estado</TD>");
		...
		//        sb.Append(tblBodyMovil.innerHTML);
		...
		for (var i = 0; i < tblBodyMovil.rows.length; i++) {
	        ...
	        for (var x = 0; x < tblBodyMovil.rows[i].cells.length; x++) {
                if (x == 16 || x == 17){
                    if (tblBodyMovil.rows[i].cells[x].className.toUpperCase() == "TDBR SV")
                        sb.Append("<td style='background-color:#00ff00;text-align:right'>");
                    else if (tblBodyMovil.rows[i].cells[x].className.toUpperCase() == "TDBR SA")
                        sb.Append("<td style='background-color:yellow;text-align:right'>");
                    else if (tblBodyMovil.rows[i].cells[x].className.toUpperCase() == "TDBR SR")
                ...
                } else sb.Append(tblBodyMovil.rows[i].cells[x].outerHTML);
	        }
	        sb.Append("</tr>");
	    }
...
function scrollTablaProy() {
    try {
	    ...
	    mostrarErrorAplicacion("Error al controlar el scroll de la tabla de profesionaless.", e.message);
    }
}
