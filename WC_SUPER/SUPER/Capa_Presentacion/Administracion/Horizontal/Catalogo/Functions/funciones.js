...
function Detalle(oFila)
{
    try{
        mostrarProcesando();        
        var strEnlace = strServer + "Capa_Presentacion/Administracion/Horizontal/Detalle/Default.aspx?ID=" + oFila.id + "&ORIGEN=ADM";
        //window.focus();
...
function Nuevo()
{
	try {
	    mostrarProcesando();
	    var strEnlace = strServer + "Capa_Presentacion/Administracion/Horizontal/Detalle/default.aspx?bNueva=true&ORIGEN=ADM";
	    //window.focus();
	    modalDialog.Show(strEnlace, self, sSize(1010, 545))
	        .then(function (ret) {
	            ObtenerDatos();
