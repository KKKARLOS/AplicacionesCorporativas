...
public partial class Capa_Presentacion_Administracion_Preventa_Tareas_Default : System.Web.UI.Page, ICallbackEventHandler
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!Page.IsCallback)
        {
            ...
            Master.TituloPagina = "Tareas de preventa";
            Master.bFuncionesLocales = true;
            if (!Page.IsPostBack)
            {
                Master.FuncionesJavaScript.Add("Javascript/draganddrop.js");
                IB.SUPER.ADM.SIC.BLL.TipoTareaPreventa oTarea = new IB.SUPER.ADM.SIC.BLL.TipoTareaPreventa();
                try
                {              
                          
                    List<IB.SUPER.ADM.SIC.Models.TipoTareaPreventa> oLista = oTarea.Catalogo();
    ...
    {
        string sResul = "OK@#@";
        IB.SUPER.ADM.SIC.BLL.TipoTareaPreventa oTarea = null;
        try
        {
            oTarea = new IB.SUPER.ADM.SIC.BLL.TipoTareaPreventa();
            sResul+= getHtmlMantenimiento(oTarea.Grabar(strFunciones)) + "@#@";


        }
        catch (Exception ex)
        {
            sResul = "Error@#@" + Errores.mostrarError("Error al actualizar los tipos de tarea de preventa.", ex, false);
        }
        finally
        {
            oTarea.Dispose();
    ...
    protected string getHtmlMantenimiento(List<IB.SUPER.ADM.SIC.Models.TipoTareaPreventa> oLista)
    {
        ...
        try
        {
            ...
            foreach (IB.SUPER.ADM.SIC.Models.TipoTareaPreventa oElem in oLista)
            {
                sb.Append("<tr id='" + oElem.ta219_idtipotareapreventa + "'");
                ...
                sb.Append("<td><input type='text' id='txtDesc' class='txtL' style='width:400px' value='" + oElem.ta219_denominacion + "' maxlength='50' onKeyUp='fm(event)'></td>");

                sb.Append("<td><input type='checkbox' style='width:13px;height:13px;' name='chkActiva' id='chkActiva' class='check' onclick='fm(event)' ");
                if (oElem.ta219_estadoactiva) sb.Append("checked=true");
                sb.Append("></td>");
