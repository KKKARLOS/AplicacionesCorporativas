...
function comprobarDatos() {
    try {
        ...
        var aNombresTarea = new Array();
        var index = 0;
        for (var i = 0; i < aFila.length; i++) {
            if (aFila[i].getAttribute("bd") != "D") {
                if (aFila[i].cells[2].children[0].value == "") {
                    mmoff("Inf", "Debes indicar la denominación del tipo de tarea", 250);
                    ms(aFila[i]);
                    aFila[i].cells[2].children[0].focus();
                    return false;
                }

                if (aNombresTarea.length > 0) {
                    if (aNombresTarea.contains(aFila[i].cells[2].children[0].value.trim().normalize().toLowerCase())) {
                        mostrarAviso("No está permitido dos o más tareas con la misma denominación.", "WarPer");
                        return false;
                    }
                }
                aNombresTarea[index] = aFila[i].cells[2].children[0].value.trim().normalize().toLowerCase();
                index++;
