...
function init() {
    try {
        ...
        if ($I("txtUnidad").value == "") {
            $I("lblArea").className = "texto";
            $I("lblArea").onclick = null;
        }
    ...
    if (aResul[1] != "OK") {
        ...
        switch (aResul[0]) {
            ...
            case "getResponsableArea":
                aG(0);
                $I('hdnIDResponsable').value = aResul[2];
                $I('txtDesResponsable').value = aResul[3];
                break;
            default:
                ocultarProcesando();
...
function comprobarDatos() {
    try {
        ...
        if ($I("hdnIdArea").value == "") {
            tsPestanas.setSelectedIndex(0);
            mmoff("War", "El área es dato obligatorio.", 240);
            return false;
        }
...
function grabarP0() {
    ...
    sb.Append($I("hdnIdArea").value + "##");  //4
    //sb.Append(getRadioButtonSelectedValue("rdbLider", true));//5
    sb.Append(($I("chkLider").checked == true) ? "1##" : "0##"); //5
    return sb.ToString();
}
function grabarP1_Old() {
    var sb = new StringBuilder;
    if (aPestGral[1].bModif) {
        //Control de las figuras
        for (var i = 0; i < $I("tblFiguras2").rows.length; i++) {
            ...
            if ($I("tblFiguras2").rows[i].getAttribute("bd") != "") {
                sbFilaAct.Append($I("tblFiguras2").rows[i].getAttribute("bd") + "##" + $I("tblFiguras2").rows[i].id + "##"); //1
                if ($I("tblFiguras2").rows[i].getAttribute("bd") == "D") {
...
function grabarP1() {
    var sb = new StringBuilder;
    if (aPestGral[1].bModif) {
        sb.Append("///");//Para el caso de que borremos todas las figuras
        //Control de las figuras
        for (var i = 0; i < $I("tblFiguras2").rows.length; i++) {
            bGrabar = false;
            sbFilaAct = new StringBuilder;
...
function getItemEstructura(nNivel) {
    try {
        ...
        switch (nNivel) {
            case 21:
                strEnlace += "&nIDPadre=-1";
                break;
            case 22:
                strEnlace += "&nIDPadre=" + $I("hdnIdUnidad").value;
                break;
        }
        //window.focus();
                ...
                if (ret != null) {
                    ...
                    switch (nNivel) {
                        case 21:
                            ...
                            $I("lblArea").className = "enlace";
                            $I("lblArea").onclick = function () { getItemEstructura(22) };
                            $I("hdnIdArea").value = "";
                            $I("txtArea").value = "";
                            break;
                        case 22:
...
function getResponsableArea(idArea) {
    try {
        var js_args = "getResponsableArea@#@" + idArea;
        mostrarProcesando();
        RealizarCallBack(js_args, "");
        return;
    } catch (e) {
        mostrarErrorAplicacion("Error al getResponsableArea.", e.message);
    }
}
//////////////  CONTROL DE PESTAÑAS  /////////////////////////////////////////////
...
function comprobarIncompatibilidades(oNuevo, aLista) {
    try {
        ...
        for (var i = 0; i < aLista.length; i++) {
            if ((oNuevo.id == "D" ||oNuevo.id == "C")
                    &&
                 (aLista[i].id == "D" || aLista[i].id == "C")) {
        ...
        mostrarErrorAplicacion("Error al comprobar las incompatibilidades de las figuras de subárea de preventa.", e.message);
    }
}
