function init(){
    try{
        ...
        {
            ocultarProcesando();
            mmoff("War","Tabla T608_RECALCULOCOSTES vacía",290);
        }
...
function mostrarTabla(){
    try{
		...
		mostrarErrorAplicacion("Error al ir a mostrar T608_RECALCULOCOSTES", e.message);
    }
}
...
function excel(){
    try {
        ...
        sb.Append("<td style='background-color: #BCD4DF;font-weight:bold; width:auto;'>Cód. Iberper</td>");
...
//        }    
        mostrarProcesando();
        //window.focus();
        modalDialog.Show(strServer + "Capa_Presentacion/Administracion/RecalculoCostes/Importar.aspx", self, sSize(400, 400))
            .then(function(ret) {
                if (ret != null) {
                    var sb = new StringBuilder;
