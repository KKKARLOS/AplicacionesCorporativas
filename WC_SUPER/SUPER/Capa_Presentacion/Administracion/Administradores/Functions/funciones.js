...
function exclusivo(objeto, strCaso) {
    switch (strCaso) {
        case 1:
            //if (objeto.checked==true && objeto.parentNode.parentNode.cells[4].children[0].checked==true)
            if (objeto.checked == true) {
                objeto.parentNode.parentNode.cells[4].children[0].checked = false;
                objeto.parentNode.parentNode.cells[5].children[0].checked = false;
            }
            break;
        case 2:
            if (objeto.checked == true) {
                objeto.parentNode.parentNode.cells[3].children[0].checked = false;
                objeto.parentNode.parentNode.cells[5].children[0].checked = false;
            }
            break;
        case 3:
    ...
    mfa(objeto.parentNode.parentNode, "U");
}
    ...
    try {
        ...
        for (var i = 0; i < $I("tblDatos2").rows.length; i++) {
            if ($I("tblDatos2").rows[i].getAttribute("bd") != "") {
                sFigura = "";
                if ($I("tblDatos2").rows[i].cells[3].children[0].checked) sFigura = "A";
                if ($I("tblDatos2").rows[i].cells[4].children[0].checked) sFigura = "S";
                if ($I("tblDatos2").rows[i].cells[5].children[0].checked) sFigura = "P";
                if (sFigura == "" && $I("tblDatos2").rows[i].getAttribute("bd") != "D") {
                    ms($I("tblDatos2").rows[i]);
                    mmoff("War", "El perfil es dato obligatorio", 200);
                    return;
                }
...
function insertarItem(oFila) {
    try {
        ...
        oCtrl3.onclick = function() { exclusivo(this, 2) };
        oNC5.appendChild(oCtrl3);

        oNC6 = oNF.insertCell(-1);
        //oNC6.setAttribute("style", "width:40px;");

        var oCtrl4 = document.createElement("input");
        oCtrl4.type = "checkbox";
        oCtrl4.className = "check";
        oCtrl4.onclick = function() { exclusivo(this, 3) };
        oNC6.appendChild(oCtrl4);
