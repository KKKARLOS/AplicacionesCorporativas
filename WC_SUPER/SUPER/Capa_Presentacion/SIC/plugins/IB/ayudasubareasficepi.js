...
(function ($) {
    ...
    function Plugin(element, options) {
        ...
        function init() {
            ...
            initModal();
            $.when(loadComboUnidades()).then(function () {
        ...
        function initModal() {
            $el.append("<style>.padfilter{padding:0 15px 0 0px}</style>");
                     ...
                     "<div class='modal-content'>" +
                         ...
                         "<div class='modal-body dual-list'>" +
                             "<div id='" + pluginName + "-divProfesionales'>" +
                                 "<div class='row'>" +
                                     "<!--Filtros de búsqueda-->" +
                                    "<div class='fk_filters col-sm-6'>" +
                                        "<div class='form-group'>" +
                                            "<div class='col-xs-2 col-sm-2 padfilter'>Unidad</div>" +
                                            "<div class='col-xs-10 col-sm-8 padfilter'>" +
                                                "<select id='" + pluginName + "-cmbUnidad' class='form-control input-sm' ></select>" +
                                            "</div>" +
                                        "</div>" +
                                        "<br />" +
                                        "<div class='form-group'>" +
                                            "<div class='col-xs-2 col-sm-2 padfilter'>Área</div>" +
                                            "<div class='col-xs-10 col-sm-8 padfilter'>" +
                                                "<select id='" + pluginName + "-cmbArea' class='form-control input-sm' ></select>" +
                                            "</div>" +
                                        "</div>" +
                                    "</div>" +
            ...
            //al seleccionar una unidad en el combo, cargar el combo de areas
            $el.find("#" + pluginName + "-cmbUnidad").on("change", function (e) {
                $el.find("#" + pluginName + "-UlProfesionales").empty();
                loadComboAreas()
            });

            //al seleccionar un area en el combo, buscar las subareas.
            $el.find("#" + pluginName + "-cmbArea").on("change", function (e) {
                $el.find("#" + pluginName + "-UlProfesionales").empty();
                search();
            });
        }

        function loadComboUnidades() {

            var html = "";
            var serviceUrl = IB.vars.strserver + "Capa_Presentacion/" + options.modulo + "/Services/Ayudas.asmx";

            var payload = {
                tipo: options.tipoAyudaUnidades,
                filtro: "",
                admin: options.admin
            }
            ...
            IB.DAL.post(serviceUrl, "Buscar", payload, null,
                function (data) {
                    if (data.length > 1)
                        data.splice(0, 0, { Key: "-1", Value: "Selecciona una unidad..." });

                    data.forEach(function (item) {
                        html += "<option value='" + item.Key + "'>" + item.Value + "</option>"
                    });

                    $el.find("#" + pluginName + "-cmbUnidad").append(html);

                    if (data.length == 1) {
                        $el.find("#" + pluginName + "-cmbUnidad option").attr("selected", "selected");
                        $el.find("#" + pluginName + "-cmbUnidad").trigger("change");
                    }
        ...
        function loadComboAreas() {
            ...
            var $cmbArea = $el.find("#" + pluginName + "-cmbArea");
            $cmbArea.find("option").remove();
            
            var payload = {
                tipo: options.tipoAyudaAreas,
                ta199_idunidadpreventa: $el.find("#" + pluginName + "-cmbUnidad option:selected").val(),
                admin: options.admin
            }

            if (payload.ta199_idunidadpreventa == "-1") return;

            IB.DAL.post(serviceUrl, "ObtenerAreasDeUnidad", payload, null,
                function (data) {
                    if (data.length > 1)
                    ...
                    if (data.length == 1) {
                        $cmbArea.find("option").attr("selected", "selected");
        ...
        function search() {
            ...
            var ta200_idareapreventa = $el.find("#" + pluginName + "-cmbArea option:selected").val();
            ...
            var payload = {
                tipo: options.tipoAyudaSubareas,
            ...
            IB.DAL.post(serviceUrl, "ObtenerSubareasDeArea", payload, null,
                function (data) {
                    data.forEach(function (item) {
