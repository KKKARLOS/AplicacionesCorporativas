...
public partial class Default : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    else if (Request.QueryString["scr"] != null)
                    {
                    ...
                    else if (Request.QueryString["iu"] == null)
                    {
                        ...
                        //aIdRed[0] = "DOVEMODA";//David Velázquez
            ...
            try
            {
                ...
                if (!bForaneo)//if (Session["IDRED"].ToString() != "")
                {
                    ...
                    if (bIdentificado)
                    {
                        ...
                        //Origen = correos de preventa
                        //Víctor manda los parámetros codificados desde los correos en base 64 ASCII
                        try
                        {
                            ...
                            switch (modulo)
                            {
                                ...
                                case "SIC":
                                    if (ht["pantalla"] != null)
                                    {
                                        if (ht["pantalla"].ToString() == "CML")
                                            strEnlace = Session["strServer"] + "Capa_Presentacion/SIC/Accion/CatalogoComoLider/Default.aspx?" + IB.SUPER.Shared.Utils.codpar("CML=true&idAccion=" + ht["idAccion"] + "") + "";
                                        else if (ht["pantalla"].ToString() == "AL")
                                            strEnlace = Session["strServer"] + "Capa_Presentacion/SIC/Accion/CatalogoPosibleLider/Default.aspx?" + IB.SUPER.Shared.Utils.codpar("AL=true&idAccion=" + ht["idAccion"] + "") + "";
                                        else if (ht["pantalla"].ToString() == "DL")
                                            strEnlace = Session["strServer"] + "Capa_Presentacion/SIC/Accion/CatalogoPdteLider/Default.aspx?" + IB.SUPER.Shared.Utils.codpar("DL=true&idAccion=" + ht["idAccion"] + "") + "";
                                        else if (ht["pantalla"].ToString() == "TP")
                                            strEnlace = Session["strServer"] + "Capa_Presentacion/SIC/Tarea/CatalogoParticipante/Default.aspx?" + IB.SUPER.Shared.Utils.codpar("TP=true&idTarea=" + ht["idTarea"] + "") + "";
                                    }
                                    break;
                                default:
        ...
        if (System.Configuration.ConfigurationManager.AppSettings["ENTORNO"].ToUpper() == "E")
            Session["ServidorSSRS"] = System.Configuration.ConfigurationManager.AppSettings["SERVIDOR_RS_E"];// http://ibsqlcorp01/reportserver?/
        else
            Session["ServidorSSRS"] = System.Configuration.ConfigurationManager.AppSettings["SERVIDOR_RS_D"];// http://ibdisdesa01/reportserver?/Reporte/
        #endregion

        try
        {
            if (User.IsInRole("PPIAP30"))
                this.hdnMostrarIAP30.Value = "S";
