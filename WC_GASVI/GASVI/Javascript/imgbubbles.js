...
jQuery.fn.imgbubbles=function(options){
	...
	return this.each(function(){ //return jQuery obj
		...
		$imgs.each(function(){
			...
			var od={width:this.offsetWidth, height:this.offsetHeight, top:this.offsetTop, left:this.offsetLeft} //original dimensions of image
			...
			var ncoords=[this.offsetLeft - od.width/4, this.offsetTop - od.height/4] //coords to move enlarged image to
			...
			if ($img.attr('alt')){ //if tooltip for image defined
				...
				//alert((ncoords[0]-tipd.width/2+nd.width/2)+" "+ (ncoords[1]-tipd.height/2));
				$tip.data("specs", {
					d: tipd,
		...
		$bubblewrap.mouseover(function(e){
			if (e.target.tagName=="IMG"){
				...
				if ($img.attr('alt')){
				    ...
				    //alert("origen: "+ eval(tipspecs.d.width/2+od.width/2) +", "+ tipspecs.d.height);
					$tip.css({zIndex:1000, visibility:'visible', left:eval(tipspecs.d.width/2+od.width/2), top:tipspecs.d.height})
		...
		$bubblewrap.mouseout(function(e){
			if (e.target.tagName=="IMG"){
				...
				$img.stop().css('zIndex', 0).animate({left:od.left, top:od.top, width:od.width, height:od.height}, setting.speed) //animate image
				if ($img.attr('alt')){
					var tipspecs=$tip.data("specs")
					$tip.css({zIndex:0, visibility:'hidden'}) //hide tip
				}
			}
		})
