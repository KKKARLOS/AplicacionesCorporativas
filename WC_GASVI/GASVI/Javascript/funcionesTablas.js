    ...
    try{
        ...
        for (var i=0, nCountLoop= oTablaAux.rows.length; i<nCountLoop;i++){
            if (document.all) 
                ...
                var t = oTabla.insertRow(-1).outerHTML;
                oTabla.insertRow(-1).outerHTML = oTablaAux.rows[i].cloneNode(true).outerHTML;
                oTablaAux.rows[i].cloneNode(true).outerHTML = t;
            }
...
function crearWord(sHTML)
{
    try{
        ...
        //que solicita permiso para ejecutar ActiveX
        ocultarProcesando();
        mostrarError("Para poder generar y abrir el archivo Word, en las políticas de seguridad de su navegador, debe permitir \n\"Inicializar y activar la secuencia de comandos de los controles de ActiveX no marcados como seguros\"\nen el apartado \"Seguridad --> Intranet local\".");
        return;
    }
    ocultarProcesando();
}
...
//    var oF1 = new Date();  
    if ($I(sIDTable) == null || $I(sIDTable).rows.length <= 0) return;
    var bRes = OrdenarTabla(sIDTable, n , desc, sTipo); 
//    var oF2 = new Date(); 
//    alert((oF2.getTime() - oF1.getTime()) / 1000 + " seg.");
    if (sFuncionScroll != null && sFuncionScroll!="")
        eval(sFuncionScroll);
...
//		    case "por": sortfn = this.sortPorcentaje; break;
	    ...
	    for (j = 0; j < this.tbody[0].rows.length; j++) {
	        ...
	        switch (TipoOrd){
	            case "": oCell.sAuxOrd = thisObject.getInnerText(this.tbody[0].rows[j].cells[thisObject.sortColumnIndex]).toLowerCase().replace(/á/g, "a").replace(/é/g, "e").replace(/í/g, "i").replace(/ó/g, "o").replace(/ú/g, "u"); break;
                case "num": 
                    oCell.sAuxOrd = thisObject.getInnerText(this.tbody[0].rows[j].cells[thisObject.sortColumnIndex]).replace(new RegExp(/\./g),"").replace(new RegExp(/\,/g),".").replace(/^\s+|\s+$/g, '');
                ...
                case "por": 
                    ...
                    break;
		        case "fec": 
		            oCell.sAuxOrd = thisObject.getInnerText(this.tbody[0].rows[j].cells[thisObject.sortColumnIndex]); 
		            if (oCell.sAuxOrd==""){
		                //oCell.sAuxOrd = "19000101";
		                oCell.sAuxOrd = 0;
		            }else{
		                //oCell.sAuxOrd = oCell.sAuxOrd.substr(6,4)+oCell.sAuxOrd.substr(3,2)+oCell.sAuxOrd.substr(0,2);
		                oCell.sAuxOrd = cadenaAfecha(oCell.sAuxOrd).getTime();
		            }
		            break;
		        case "mes": 
		            oCell.sAuxOrd = thisObject.getInnerText(this.tbody[0].rows[j].cells[thisObject.sortColumnIndex]); 
	...
	this.sortCaseInsensitive = function(a,b) {
		if (a.cells[thisObject.sortColumnIndex].sAuxOrd<b.cells[thisObject.sortColumnIndex].sAuxOrd) return -1;
		if (a.cells[thisObject.sortColumnIndex].sAuxOrd==b.cells[thisObject.sortColumnIndex].sAuxOrd) return 0;
		return 1;
	}
	...
	this.sortNumeric = function(a,b) {
		return parseFloat(a.cells[thisObject.sortColumnIndex].sAuxOrd)-parseFloat(b.cells[thisObject.sortColumnIndex].sAuxOrd);
	}
...
function setExcelImg(sImg, sDiv, sFun){ 
    try{
        ...
        oImg.setAttribute("style","position:absolute;top:"+(getTop($I(sDiv))+ 17) +"px;left:"+(getRight($I(sDiv))+16)+"px;zIndex:"+$I(sDiv).style.zIndex+";height:16px;width:16px;border-width:0px;");
        
        eval("oImg.onclick=function() {mostrarProcesando();setTimeout('"+ sFuncion +"()',500);}");

        oImg.className="MANO";
        //oImg.style.position="absolute";        
        //oImg.style.top = getTop($I(sDiv)) + 17 ;
        //oImg.style.left = getRight($I(sDiv))+ 16;
...
//function setExcelImgFloat(sImg, sTblDivTitulo, sFun){ 
//    try{
//        var sFuncion = (sFun!=null) ? sFun : "excel";
//        //var oImg = document.createElement("<img id='"+ sImg +"_exp' title='Exporta a Excel el contenido de la tabla' class='MANO' onclick=\"mostrarProcesando();setTimeout('"+ sFuncion +"()',20);\" src='"+ strServer +"images/imgExcelAnim.gif' style='height:16px;width:16px;border-width:0px;margin-left:1px;float:right;margin-top:-16px;' />");
//        var oImg = document.createElement("<img id='"+ sImg +"_exp' title='Exporta a Excel el contenido de la tabla' class='MANO' onclick=\"mostrarProcesando();setTimeout('"+ sFuncion +"()',20);\" src='"+ strServer +"images/imgExcelAnim.gif' style='height:16px;width:16px;border-width:0px;margin-left:1px;float:left;margin-top:1px;' />");
//        $I(sTblDivTitulo).style.styleFloat = "left";
//        $I(sTblDivTitulo).insertAdjacentElement("afterEnd", oImg);
//	}catch(e){
//		mostrarErrorAplicacion("Error al posicional la imagen de Excel.", e.message);
//    }
//}
