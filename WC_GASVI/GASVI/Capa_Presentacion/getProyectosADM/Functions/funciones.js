...
function aceptarClick(oControl){
    try{
        ...
        strRetorno = oFila.id + "@#@" + oFila.cells[3].innerText + "@#@" + oFila.getAttribute("estado") + "@#@" + oFila.getAttribute("idPSN") + "@#@" + oFila.getAttribute("resp") + "@#@" + oFila.getAttribute("cr") ;
    ...
    if (aResul[1] != "OK"){
        ...
        switch (aResul[0]){
            case "buscar":
                ...
                scrollTablaProy();
                actualizarLupas("tblTitulo", "tblCatProy");
                window.focus();
                break;
            case "setPSN":
                aceptarSalir();
...
function scrollTablaProy(){
    try{
        ...
        if ($I("divCatalogo").offsetHeight != null)
            nUltFila = Math.min(nFilaVisible + $I("divCatalogo").offsetHeight / 20 + 1, $I("tblCatProy").rows.length);
        else
            nUltFila = Math.min(nFilaVisible + $I("divCatalogo").innerHeight / 20 + 1, $I("tblCatProy").rows.length);

        var oFila;
        for (var i = nFilaVisible; i < nUltFila; i++){
            if (!tblCatProy.rows[i].getAttribute("sw")){
                oFila = tblCatProy.rows[i];
