...
public partial class Default : System.Web.UI.Page, ICallbackEventHandler
{
    ...
    public string sOrigen = "", strTablaHTMLOficinas = "";
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            Master.TituloPagina = "Mantenimiento de oficinas de liquidación";
            ...
            if (!Page.IsCallback)
            {
                if (!Page.IsPostBack)
                {
                    try
                    {
                        strTablaHTMLOficinas = OFICINA.mostrarOficinas();
                    }
                    catch (Exception ex)
                    {
                        Master.sErrores = Errores.mostrarError("Error al obtener los datos de las oficinas de liquidación.", ex);
    ...
    public void RaiseCallbackEvent(string eventArg)
    {
        ...
        switch (aArgs[0])
        {
            case ("grabar"):
                try
                {
                    OFICINA.Grabar(aArgs[1]);
                    sResultado += "OK@#@";
                }
                catch (Exception ex)
                {
                    sResultado += "Error@#@" + Errores.mostrarError("Error al grabar las oficinas de liquidación.", ex);
                }
                break;
        }
