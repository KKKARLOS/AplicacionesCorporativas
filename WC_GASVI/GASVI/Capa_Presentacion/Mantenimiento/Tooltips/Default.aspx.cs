...
public partial class Default : System.Web.UI.Page, ICallbackEventHandler
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            Master.TituloPagina = "Mantenimiento de textos de ayuda";
            ...
            if (!Page.IsCallback)
            {
                if (!Page.IsPostBack)
                {
                    try
                    {
                        txtDescripcion.Text = Tooltips.ObtenerTexto("0");
                        hdnOrigen.Text = "0";
                    }
                    catch (Exception ex)
                    {
                        Master.sErrores = Errores.mostrarError("Error al obtener los datos la descripción del tooltip.", ex);
    ...
    public void RaiseCallbackEvent(string eventArg)
    {
        ...
        switch (aArgs[0])
        {
            case ("grabar"):
                try
                {
                    Tooltips.Grabar(aArgs[1], aArgs[2]);
                    sResultado += "OK@#@";
                }
                catch (Exception ex)
                {
            ...
            case ("verTooltip"):
                try
                {
                    sResultado += "OK@#@" + Tooltips.ObtenerTexto(aArgs[1]);
                }
                catch (Exception ex)
                {
