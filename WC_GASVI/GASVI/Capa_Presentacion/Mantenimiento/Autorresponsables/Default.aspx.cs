...
public partial class Default : System.Web.UI.Page, ICallbackEventHandler
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            Master.TituloPagina = "Mantenimiento de autorresponsables";
            ...
            if (!Page.IsCallback)
            {
                ...
                if (!Page.IsPostBack)
                {
                    try
                    {
                        strTablaHTMLIntegrantes = AUTORRESPONSABLE.CatalogoIntegrantes(); //Rellena la tabla con todas los integrantes
                    }
                    catch (Exception ex)
                    {
    ...
    public void RaiseCallbackEvent(string eventArg)
    {
        ...
        switch (aArgs[0])
        {
            case ("grabar"):
                try
                {
                    AUTORRESPONSABLE.Grabar(aArgs[1]);
                    sResultado += "OK@#@";
                }
                catch (Exception ex)
                {
            ...
            case ("buscar"):
                try
                {
                    sResultado += "OK@#@" + AUTORRESPONSABLE.obtenerPersonas(aArgs[1], aArgs[2], aArgs[3], aArgs[4]);
                }
                catch (Exception ex)
                {
