    ...
    try{
	    ...
	    if (sFechaHasta != ""){
	        ...
	        if ($I("tblMesFin2_td_"+ dHasta.ToShortDateString().replace(reg,"")) != null){
	            if (ie) $I("tblMesFin2_td_"+ dHasta.ToShortDateString().replace(reg,"")).click();
                else {
                    var clickEvent = window.document.createEvent("MouseEvent");
                    clickEvent.initEvent("click", false, true);
                    $I("tblMesFin2_td_"+ dHasta.ToShortDateString().replace(reg,"")).dispatchEvent(clickEvent);
                }
            }
	    }
        ...
        if (oCelda.parentNode.parentNode.parentNode.id.indexOf("Ini") > -1){ //Inicio
            ...
            if (oCeldaSelHasta == null){
                ...
                if ($I(sCeldaSelHastaAux) != null){
                    if (ie) $I(sCeldaSelHastaAux).click();
                    else {
                        var clickEvent = window.document.createEvent("MouseEvent");
                        clickEvent.initEvent("click", false, true);
                        $I(sCeldaSelHastaAux).dispatchEvent(clickEvent);
                    }
                }
            ...
            if (oCeldaSelDesde == null){
                ...
                if ($I(sCeldaSelDesdeAux) != null){
                    if (ie) $I(sCeldaSelDesdeAux).click();
                    else {
                        var clickEvent = window.document.createEvent("MouseEvent");
                        clickEvent.initEvent("click", false, true);
                        $I(sCeldaSelDesdeAux).dispatchEvent(clickEvent);
                    }
                }
    ...
    try{
        ...
        if (sOpcion == "I"){
            ...
            if ($I("tblMesFin2_td_"+oCeldaSelHasta.split("_")[2]) != null){
                if (ie) $I("tblMesFin2_td_"+oCeldaSelDesde.split("_")[2]).click();
                else {
                    var clickEvent = window.document.createEvent("MouseEvent");
                    clickEvent.initEvent("click", false, true);
                    $I("tblMesFin2_td_"+oCeldaSelDesde.split("_")[2]).dispatchEvent(clickEvent);
                }
...
//        window.returnValue = sDesdeAux +"@#@"+ sHastaAux;
//        window.close();
