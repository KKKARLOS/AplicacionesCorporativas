...
namespace SII.HCD.Administrative.BL
{
    public class CustomerBL : PersonBL, ICustomerService
    {
        ...
        //    try
        //    {
        //        CustomerEntity oldCustomerEntity = GetCustomer_old(customerID, false, false, false);
        //        CustomerEntity newCustomerEntity = GetCustomer_new(customerID, false, false, false);

        //        bool res = newCustomerEntity.CompareEquals(oldCustomerEntity);
        ...
        //public CustomerEntity GetCustomer_old(int customerID, bool getNOKs, bool getCCPs, bool getCCOs)
        //{
        //    try
        //    {
                ...
                if ((ds.Tables != null) && (ds.Tables.Contains(Administrative.Entities.TableNames.CustomerTable)) && (ds.Tables[Administrative.Entities.TableNames.CustomerTable].Rows.Count > 0))
                {
                    ...
                    var tCCOs = System.Threading.Tasks.Task.Factory.StartNew(() =>
                    {
                        try
                        {
                            if (getCCOs)
                            {
                                if ((ds != null) && (ds.Tables.Contains(SII.HCD.Administrative.Entities.TableNames.CustomerContactOrganizationListDTOTable)))
                                {
                                    foreach (DataRow row in ds.Tables[SII.HCD.Administrative.Entities.TableNames.CustomerContactOrganizationListDTOTable].Rows)
                                    {
                                        int customerContactOrganizationID = SIIConvert.ToInteger(row["ID"].ToString(), 0);
                                        if (customerContactOrganizationID > 0)
                                            customerContactOrganizations.Add(CustomerContactOrganizationBL.GetCustomerContactOrganization(customerContactOrganizationID));
        ...
        //public CustomerEntity GetCustomerByPersonID_old(int personID)
        //{
        //    try
        //    {
        //        PersonBL personBL = new PersonBL();
        //        DataSet ds = DataAccess.CustomerDA.GetCustomerByPersonID_old(personID);
        ...
        //    try
        //    {
        //        CustomerEntity oldCustomerEntity = GetCustomerByPersonID_old(personID);
        //        CustomerEntity newCustomerEntity = GetCustomerByPersonID_new(personID);
