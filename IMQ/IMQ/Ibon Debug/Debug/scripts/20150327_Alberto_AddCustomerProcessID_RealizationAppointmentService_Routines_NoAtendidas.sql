...
select RAI.[ID] ReceptionInfoAppointmentRelID, RA.[ID] ActID, CR.CustomerProcessID
FROM CustomerReception CR 
	...
	JOIN RoutineAct RA ON RA.CustomerOrderRequestID=COR.[ID]
...
BEGIN
	...
	SET @RASID = ISNULL((select TOP 1 RAS.[ID] RealizationAppointmentService
						FROM ReceptionAdditionalInfo RAI 
							...
							JOIN CustomerOrderRequest COR ON Apps.AppointmentElement=3 AND COR.[ID]=Apps.ElementID
							JOIN RoutineAct RA ON RA.CustomerOrderRequestID=COR.[ID]
							JOIN RealizationAppointmentService RAS ON RA.[ID]=RAS.ElementID AND RAS.AppointmentElement=2 AND (RAS.CustomerProcessID=0)
						...
						WHERE (RAi.[ID]=@ReceptionAdditionalInfoID) AND (RA.[ID]=@ActID)
