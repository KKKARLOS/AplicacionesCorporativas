...
document.onreadystatechange = function() {
    if (document.all) {
        ...
        try {
            if (document.all("divPestRetr") != null) {
                if (document.readyState == "complete") {
                    oPestRetr = document.all("divPestRetr");
                    oImgPestana = document.all("imgPestHorizontalAux");
                }
            }
        } catch (e) { }
    }
...
function __PopCalSetFocus(o) {
    o.keyboard = true
    o.eventKey = 0
    /* Código modificado para saber cuando se cambia el valor del input para poder realizar alguna acción */
    if (document.all) {
...
function PopCalInitCalendar(l) {
    ...
    if (PopCal) {
        if (PopCal.initialized == 0) {
            ...
            if (oPC.showToday == 1) {
                sCalendar += "<tr><td style='padding:3px;' Class='TodayStyle' align=center><span id='popupSuperToday" + oPC.id + "'></span></td></tr>"
            }
            sCalendar += "</table></div>"
...
function PopCalMoveDefaultPos(l) {
    ...
    if (PopCal.keepInside == 1) {
        ...
        if (((toppos + PopCal.popupSuperCalendar.offsetHeight + 140) - aTag.scrollTop) > aTag.offsetHeight) {
