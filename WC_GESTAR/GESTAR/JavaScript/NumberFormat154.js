...
//Salidas:  Si la tecla pulsada no es válida, anula su valor.  
    ...
    var strValorDelObjeto = desformatearNumero(objActivo.value);
    var intNumEnteros;
...
//Mikel 05/01/2007. validación de teclas en campos con numeros enteros
//Finalidad:Testea la tecla pulsada. 
//Salidas:  Si la tecla pulsada no es válida, anula su valor.  
    var strTeclaPulsada=event.keyCode;
    //alert("strTeclaPulsada: "+strTeclaPulsada);
...
function desformatearNumero (strDato){
...
function desformatNumero(strDato){
    var reg = /\./g;
    sValor = strDato.replace(reg,"");
    reg = /\,/g;
    sValor = sValor.replace(reg,".");
    return sValor;
}

/***********************************************
Función: desformatNum
Objetivo: Eliminar los puntos y convertir la coma en punto
Inputs: obj		--> control a desformatear;
Output: Número desformateado
************************************************/
function desformatNum(obj){
...
Inputs: obj		--> control a formatear;
		nNumDec --> número de decimales
		bTruncar--> true (truncar) false (redondear)
Output: Número formateado
************************************************/
function formatearNumero(obj, nNumDec, bTruncar){
    //alert(obj.tagName);
    var swObj = 0;
	...
	num.setInputDecimal('.');
	num.setNumber(str); // obj.value is '1000.247'
	num.setPlaces(nNumDec, bTruncar);
	...
	if (swObj == 0)
        ...
        obj.innerText = num.toFormatted();
}

/***********************************************
Función: formatearFloat
Inputs: obj		--> Float a formatear;
		nNumDec --> número de decimales
		bTruncar--> true (truncar) false (redondear)
Output: Número formateado
************************************************/
function formatearFloat(nValue, nNumDec, bTruncar){
	var num = new NumberFormat();
	num.setInputDecimal('.');
	num.setNumber(nValue); // obj.value is '1000.247'
	num.setPlaces(nNumDec, bTruncar);
	num.setNegativeFormat(num.LEFT_DASH);
	num.setSeparators(true, '.', ',');
